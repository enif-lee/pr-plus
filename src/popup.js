/**
 * AUTO-GENERATED from src/popup.ts
 * SOURCE OF TRUTH: src/popup.ts — do not edit this .js
...
function normalizeHostInput(raw) {
  return String(raw || "").trim().toLowerCase().replace(/^https?:\/\//, "").replace(/\/.*$/, "").replace(/:\d+$/, "");
}
function updateEndpointPreview(hostRaw) {
  ...
  if (webHost === "github.com" || webHost === "www.github.com") {
    ...
    const apiHost = webHost === "ghe.com" || webHost.startsWith("api.") ? webHost === "ghe.com" ? "api.ghe.com" : webHost : `api.${webHost}`;
    rest = `https://${apiHost}`;
  ...
  endpointPreviewEl.textContent = `Preview for ${webHost}: REST ${rest} \xB7 GraphQL ${gql}`;
}
...
function renderHostAccounts(accounts) {
  ...
  const previewHost = (
    ...
    normalizeHostInput(enterpriseHostInput?.value) || hostAccountsState[0]?.host || "github.com"
  );
...
function updateAddHostButtonState() {
  ...
  addHostAccountBtn.textContent = isUpdate ? "Update PAT & grant access" : "Add host & grant access";
...
async function load() {
  try {
    ...
    const accounts = hostsRes?.accounts || prefsRes?.hostAccounts || [];
...
addHostAccountBtn?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (perm && perm.granted === false) {
      setEnterpriseStatus(
        perm.error || "Permission denied \u2014 grant access to the enterprise host when prompted.",
        true
      );
    } else {
      setEnterpriseStatus(
        `${isUpdate ? "Updated" : "Saved"} ${host}${reg?.registered ? " \xB7 content scripts registered" : ""}`
      );
    }
...
clearIdbBtn?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (tabs === 0) {
      ...
      setStatus(
        `Cleared cache on ${cleared} tab${cleared === 1 ? "" : "s"}${tabs > cleared ? ` (${tabs - cleared} skipped)` : ""}`
      );
    }
  } catch (err) {
