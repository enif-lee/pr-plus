/**
 * Check-run / commit-status helpers (TypeScript SoT).
 * Built to src/modal/pure/checks.js via npm run build:pure.
 */
...
export function parseTime(value: any) {
...
export function statusKey(s: any) {
...
export function statusTime(s: any) {
  return (
 ...
 * @returns {Array}
 */
export function distinctStatuses(statuses: any) {
...
export function checkRunKey(r: any) {
...
export function checkRunTime(r: any) {
  return (
 ...
 * @returns {Array}
 */
export function distinctCheckRuns(runs: any) {
 ...
 */
export function deriveChecksState(statuses: any, checkRuns: any, fallback: any = 'unknown') {
 ...
 */
export function normalizeChecks(checks: any) {
 ...
 */
export function statusIsInProgress(item: any) {
 ...
 */
export function classifyCheckOutcome(item: any) {
...
export function formatRelativeAgo(iso: any, nowMs: any) {
 ...
 * @param {number} [nowMs]
 */
export function formatCheckSummary(item: any, nowMs: any) {
  if (!item) return '';
 ...
 * @param {object|null|undefined} checks
 */
export function checksNeedElapsedTick(checks: any) {
 ...
 */
export function buildMergeBoxCheckGroups(checks: any, opts: any) {
  const nowMs =
  ...
  for (const row of rows) {
    const b = (buckets as any)[row.outcome] || buckets.pending;
    b.push(row);
  }

  /** @type {Array<{ key: string, label: string, outcome: string, items: Array }>} */
  const groups: any[] = [];
  const pushGroup = (key: any, outcome: any, singular: any, plural: any, items: any) => {
...
export function mergeBoxChecksHeadline(state: any, totalCount: any) {
 ...
 */
export function summarizeCheckCounts(checks: any) {
  const n = normalizeChecks(checks);
  let success = 0;
  let failure = 0;
  let pending = 0;
 ...
 */
export function formatChecksCountLabel(summary: any) {
 ...
 */
export function listCheckNamesByOutcome(checks: any) {
  const n = normalizeChecks(checks);
 ...
 */
export function formatCheckGroupTip(outcome: any, names: any, locale: any) {
  const list = Array.isArray(names) ? names.filter(Boolean) : [];
  const n = list.length;
  const pure =
    typeof globalThis !== 'undefined'
      ? (globalThis as { PRModalI18n?: { formatMessage?: (...a: any[]) => string } }).PRModalI18n
      : null;
  const t = (key: any, subs?: any) => {
