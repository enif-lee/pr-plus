/**
 * AUTO-GENERATED from src/modal/lib/command-palette.ts
 * SOURCE OF TRUTH: src/modal/lib/command-palette.ts
...
__export(stdin_exports, {
  PR_MODAL_OPT_ACTIONS: () => PR_MODAL_OPT_ACTIONS,
  buildDiffPaletteCommands: () => buildDiffPaletteCommands,
  buildPaletteCommands: () => buildPaletteCommands,
  buildPrModalOptHoldSlots: () => buildPrModalOptHoldSlots,
  filterPaletteCommands: () => filterPaletteCommands,
  formatShortcut: () => formatShortcut,
  optShortcutForCommandId: () => optShortcutForCommandId,
  resolveAdjacentPrNumber: () => resolveAdjacentPrNumber,
  resolveDevelopmentMainOpen: () => resolveDevelopmentMainOpen,
  resolvePrModalOptAction: () => resolvePrModalOptAction,
  stackDigitSlotNumber: () => stackDigitSlotNumber
});
...
function resolvePrModalOptAction(opts = {}) {
  ...
  if (/^(Shift|Alt|Meta|Control)(Left|Right)?$/i.test(code) || /^(Shift|Alt|Meta|Control)$/i.test(String(opts.key || ""))) {
    return null;
  }
  const pack = (def) => ({
    id: def.id,
...
function buildDiffPaletteCommands() {
  return [
    {
      id: "diff-prev-file",
      title: "Previous file",
    ...
    },
    {
      ...
      shortcut: optShortcutForCommandId("diff-sel-extend-down") || "shift+arrowdown",
...
function resolveDevelopmentMainOpen(item, ctx = {}) {
  ...
  const fallbackHref = href || (Number.isFinite(num) && num > 0 && owner && repo ? `https://github.com/${owner}/${repo}/issues/${num}` : "");
  ...
  const knownPulls = Array.isArray(ctx?.knownPullNumbers) ? ctx.knownPullNumbers.map(Number).filter((n) => Number.isFinite(n) && n > 0) : [];
  const knownAsPull = knownPulls.includes(num);
  const isPull = kind === "pull" || kind === "pullrequest" || urlIsPull || knownAsPull || Boolean(ctx?.preferInModal) && !urlIsIssue && kind !== "issue";
  if (isPull) {
    return {
      mode: "inModal",
      number: num,
      href: href || (owner && repo ? `https://github.com/${owner}/${repo}/pull/${num}` : fallbackHref)
    };
  }
  return {
    mode: "navigate",
...
function resolveAdjacentPrNumber(opts = {}) {
  ...
  const stack = (Array.isArray(opts.stackItems) ? opts.stackItems : []).map((x) => Number(x?.number)).filter((n) => Number.isFinite(n) && n > 0);
  if (stack.length >= 2) {
    const idx2 = stack.indexOf(current);
    if (idx2 < 0) return null;
    const next2 = stack[idx2 + dir];
    return next2 != null && Number.isFinite(next2) ? next2 : null;
  }
  const pulls = (Array.isArray(opts.openPulls) ? opts.openPulls : []).map((x) => Number(x?.number ?? x)).filter((n) => Number.isFinite(n) && n > 0);
...
function buildPaletteCommands(detail, opts = {}) {
  ...
  const allowReviewVerdict = opts.canSubmitReviewVerdict != null ? Boolean(opts.canSubmitReviewVerdict) : (() => {
    const a = String(d.author || "").trim().replace(/^@/, "").toLowerCase();
    const v = String(d.viewerLogin || "").trim().replace(/^@/, "").toLowerCase();
    return !(a && v && a === v);
  })();
  const cmds = [
    {
    ...
    },
    {
      ...
      title: isDiffLayout ? "Reveal focused thread in Diff" : "Go to Diff for focused thread",
      section: "Review",
    ...
    ...!isDiffLayout ? [
      ...
      },
      {
        ...
        shortcut: optShortcutForCommandId("conv-scroll-down") || "opt+arrowdown",
      ...
      },
      {
        ...
        section: "Conversation",
        keywords: ["scroll", "page", "down"],
        shortcut: optShortcutForCommandId("conv-page-down") || "opt+shift+arrowdown",
  ...
  cmds.push({
    ...
    description: stacked ? "Move up the stack path" : "Open previous PR in the pulls list",
  ...
  cmds.push({
    ...
    description: stacked ? "Move down the stack path" : "Open next PR in the pulls list",
...
function filterPaletteCommands(commands, query) {
  ...
  return list.filter((c) => {
    const hay = [c.title, c.section, ...c.keywords || [], c.id].filter(Boolean).join(" ").toLowerCase();
    return hay.includes(q) || q.split(/\s+/).every((w) => hay.includes(w));
  }).slice(0, 50);
}
function formatShortcut(shortcut, isMac = false) {
  if (!shortcut) return "";
  return String(shortcut).replace(/mod\+/gi, isMac ? "\u2318" : "Ctrl+").replace(/shift\+/gi, isMac ? "\u21E7" : "Shift+").replace(/opt\+|alt\+/gi, isMac ? "\u2325" : "Alt+").replace(/enter/gi, "\u21B5");
}
  ...
  global.PRModalCommandPalette = api;
