...
function buildPathOrderMap(filesOrPaths) {
  ...
  for (const item of filesOrPaths) {
    const p =
      typeof item === 'string' ? item : (item && (item.filename || item.path)) || '';
...
function sortMappedCommentsByDiffOrder(mapped, pathOrder = null) {
  ...
  const order = pathOrder instanceof Map ? pathOrder : buildPathOrderMap(pathOrder);
  list.sort((a, b) => {
    const ra = a && a.rowIndex;
    const rb = b && b.rowIndex;
    ...
    const la = !a || a.line == null ? Number.MAX_SAFE_INTEGER : Number(a.line);
    const lb = !b || b.line == null ? Number.MAX_SAFE_INTEGER : Number(b.line);
    if (la !== lb) return la - lb;
...
/**
 * Attach rowIndex by matching virtual rows; result ordered top → bottom in Diff.
 */
function mapCommentsToRowIndices(comments, virtualRows, opts = {}) {
  const pathOrder = (opts && opts.pathOrder) || null;
