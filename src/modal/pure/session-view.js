...
const OPEN_MODAL_MAX_AGE_MS = 12 * 60 * 60 * 1000;
 ...
 * @param {{ owner: string, repo: string, number: number|string }} payload
 */
...
function parseOpenModal(raw, opts = {}) {
  ...
  const page =
    pageRaw === 'diff' ? 'diff' : pageRaw === 'conversation' ? 'conversation' : null;
  const position =
...
if (typeof globalThis !== 'undefined') {
  globalThis.PRModalSessionView = api;
}
