 ...
 * UI still consumes a flat app-detail via toAppDetail() (projection only).
 *
 * Slices:
 *   meta          — PR identity + people/labels/milestone/branch/merge stats
 *   files         — changed files + patches
 *   commits       — commit list
 *   comments      — issue conversation comments
 *   reviews       — submitted PR reviews
 *   checks        — CI status / check runs
 *   development   — linked issues / projects
 *   threads       — review threads + review comments
 *   pendingReview — viewer PENDING review shell
 */
(function (global) {
  const META_KEYS = [
    'owner',
