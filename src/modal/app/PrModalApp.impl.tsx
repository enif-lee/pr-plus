...
export function PrModalApp({
  ...
  function runPaletteCommand(cmd: any) {
    runPaletteCommandImpl({
      ...
      toggleFullscreen, startEditTitle: () => setTitleEditSignal((n)=>n+1), startEditBody: () => setEditingBody(true),
      openBasePicker, onSetDraftStage, onMergePr, onUpdateBranch, onSubscribe, onUnsubscribe,
      openMilestonePicker: () => openMilestonePicker?.(), clearMilestone: () => void applyMilestone?.(null),
      onRerequestReview, openReviewerPicker, openAssigneePicker, onRemoveReviewer, onRemoveAssignee,
      openLabelPicker, onLeaveReviewAction, onClosePr, onReopenPr, applySetLabels, detail, layoutMode, LAYOUT_DIFF,
      applyActionRef, setActionMsg, focusCommentBox, collapseActiveFile: () => setActiveFileCollapse(true),
      expandActiveFile: () => setActiveFileCollapse(false), collapseFold, expandFold, stepNavPrev: () => navComment(-1),
      stepNavNext: () => navComment(1), scrollDiffPage, optArrowScrollSelect, toggleViewedActiveFile,
      toggleActiveFileCollapse, contextThreadCollapse, contextThreadExpand, contextThreadFold,
      contextThreadGotoDiff, contextThreadComment, contextThreadResolve, focusedThreadFold,
      focusedThreadGotoDiff, focusedThreadComment, focusedThreadResolve,
      scrollConversationPanel, navConversationComment, navFile,
