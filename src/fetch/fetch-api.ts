/**
 * SOURCE OF TRUTH — complete GitHub fetch API.
 * Runtime: src/fetch-pulls.js via npm run build:fetch
 ...
 */
function mapApiPullRequest(pr: any) {
...
function buildApiHeaders(token: any) {
...
function findDanglingPrNumbers(pagePrNumbers: any, prs: any) {
...
async function mapWithConcurrency(items: any, limit: any, worker: any) {
...
async function fetchOpenPullsPublic(owner: any, repo: any, fetchImpl: any, token: any = null, ctx: any = null) {
 ...
 */
async function fetchDanglingPulls(owner: any, repo: any, numbers: any, fetchImpl: any, token: any = null, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
 ...
 */
async function fetchRepoAutolinks(owner: any, repo: any, fetchImpl: any, token: any = null, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
 ...
 */
function buildAutolinkUrl(urlTemplate: any, num: any) {
 ...
 */
function matchAutolinksInText(text: any, autolinks: any) {
 ...
 */
function attachMagicLinks(prs: any, autolinks: any) {
 ...
 */
async function fetchOpenPulls(owner, repo, fetchImpl, options: any = {}) {
  const {
    token = null,
    pagePrNumbers = [],
...
function decodeBase64Utf8(b64: any) {
  try {
 ...
 */
async function apiJsonWithLink(url: any, fetchImpl: any, token: any) {
  const res = await fetchImpl(url, { headers: buildApiHeaders(token) });
  if (!res.ok) {
    const err = new Error(`GitHub API ${res.status}: ${res.statusText}`);
    err.status = res.status;
...
function parseLinkNextUrl(linkHeader: any) {
 ...
 */
async function fetchRestCollectionAll(firstUrl, fetchImpl, token, opts: any = {}) {
  const maxPages =
 ...
 */
async function fetchPrCommits(owner: any, repo: any, number: any, fetchImpl: any, token: any = null, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
 ...
 */
async function fetchAllPrCommits(owner: any, repo: any, number: any, fetchImpl: any, token: any = null, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  const o = String(owner || '').trim();
  const r = String(repo || '').trim();
  const n = Number(number);
 ...
 */
async function fetchPrChecks(owner: any, repo: any, headSha: any, fetchImpl: any, token: any = null, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  const o = String(owner || '').trim();
  const r = String(repo || '').trim();
 ...
 */
async function fetchPrIssueComments(owner: any, repo: any, number: any, fetchImpl: any, token: any = null, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  const o = String(owner || '').trim();
  const r = String(repo || '').trim();
  const n = Number(number);
 ...
 * @returns {Promise<Array>}
 */
async function fetchPrReviews(owner: any, repo: any, number: any, fetchImpl: any, token: any = null, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  const o = String(owner || '').trim();
  const r = String(repo || '').trim();
  const n = Number(number);
...
function mapReviewComment(c, extra: any = {}) {
 ...
 */
function mapGraphqlReviewCommentNode(node, threadMeta: any = {}) {
  if (!node) return null;
 ...
 */
function mergePendingReviewComments(published: any, pendingList: any) {
 ...
 */
function pickViewerPendingFromReviews(reviews: any, login: any) {
 ...
 */
async function fetchViewerPendingReviewBundle(owner: any, repo: any, pullNumber: any, fetchImpl: any, token: any, preloaded: any = null, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
...
async function fetchViewerPendingReviewComments(owner: any, repo: any, pullNumber: any, fetchImpl: any, token: any) {
 ...
 */
async function deletePendingPullReview(owner: any, repo: any, pullNumber: any, reviewId: any, fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  const id = Number(reviewId);
  if (!Number.isFinite(id) || id <= 0) {
    throw new Error('Invalid pending review id');
 ...
 */
async function fetchPrCommentsPage(owner: any, repo: any, pullNumber: any, kind: any, opts: any, fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
 ...
 */
async function fetchPrSidebarMeta(owner: any, repo: any, number: any, fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  const o = String(owner || '').trim();
  const r = String(repo || '').trim();
  const n = Number(number);
 ...
 */
async function fetchIssueOrPrSummaries(owner: any, repo: any, numbers: any, fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  const o = String(owner || '').trim();
  const r = String(repo || '').trim();
  const nums = [
 ...
 * @param {string} token
 */
async function fetchReviewThreadsByIds(threadNodeIds: any, fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  const empty = {
    threads: [],
    comments: [],
 ...
 * @returns {object|null}
 */
function dropReviewThreadsFromDetail(detail: any, threadNodeIds: any) {
 ...
 */
function mergeReviewThreadsPageIntoDetail(detail: any, page: any, direction: any = 'older') {
  if (!detail) return detail;
...
async function fetchPullReviewThreads(owner: any, repo: any, pullNumber: any, fetchImpl: any, token: any) {
  try {
 ...
 * @returns {Promise<T>}
 */
async function timedFetch(timings: any, name: any, promise: any, extra: any = undefined, opts: any = {}) {
  const t0 = fetchNowMs();
  const batchStart =
 ...
 */
function logParallelRestSummary(timings: any, names: any, wallMs: any) {
 ...
 */
async function fetchConflictFilePaths(owner: any, repo: any, baseRef: any, headSha: any, fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  const o = String(owner || '').trim();
  const r = String(repo || '').trim();
...
async function postIssueComment(owner: any, repo: any, issueNumber: any, body: any, fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  return apiSend(
 ...
 */
async function findViewerPendingReview(owner: any, repo: any, pullNumber: any, fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
...
function mapGraphqlReviewCommentToRest(c, fallback: any = {}) {
  if (!c) return null;
  return {
 ...
 */
async function replyViaThreadGraphql(threadNodeId, body, fetchImpl, token, fallback: any = {}, ctx = null) {
  ctx = normalizeApiCtx(ctx);
  const data = await apiGraphql(
 ...
 */
async function resolveParentCommentNodeId(owner: any, repo: any, parentId: any, fetchImpl: any, token: any, knownNodeId: any, pullNumber: any = null, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
 ...
 */
async function resolveReviewThread(threadNodeId: any, resolved: any, fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
 ...
 */
async function updatePullState(owner: any, repo: any, pullNumber: any, state: any, fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
...
async function closePullRequest(owner: any, repo: any, pullNumber: any, fetchImpl: any, token: any) {
  return updatePullState(owner, repo, pullNumber, 'closed', fetchImpl, token);
}

async function reopenPullRequest(owner: any, repo: any, pullNumber: any, fetchImpl: any, token: any) {
 ...
 */
async function deleteReviewComment(owner: any, repo: any, commentId: any, fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  return apiSend(
 ...
 */
async function deleteIssueComment(owner: any, repo: any, commentId: any, fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  return apiSend(
...
async function editIssueComment(owner: any, repo: any, commentId: any, body: any, fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  return apiSend(
    githubRestUrl(`/repos/${owner}/${repo}/issues/comments/${commentId}`, ctx),
    fetchImpl,
...
async function editReviewComment(owner: any, repo: any, commentId: any, body: any, fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  return apiSend(
    githubRestUrl(`/repos/${owner}/${repo}/pulls/comments/${commentId}`, ctx),
    fetchImpl,
...
async function addAssignees(owner: any, repo: any, issueNumber: any, assignees: any, fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  return apiSend(
...
async function removeAssignees(owner: any, repo: any, issueNumber: any, assignees: any, fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  return apiSend(
    githubRestUrl(`/repos/${owner}/${repo}/issues/${issueNumber}/assignees`, ctx),
    fetchImpl,
...
async function setIssueLabels(owner: any, repo: any, issueNumber: any, labels: any, fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  return apiSend(
 ...
 */
async function fetchRepoLabels(owner, repo, fetchImpl, token = null, opts: any = {}) {
  const ctx = normalizeApiCtx(opts?.ctx);
  const perPage = 100;
...
function defaultNewLabelColor(name: any) {
  const palette = [
    'd73a4a',
    '0075ca',
    'a2eeef',
 ...
 */
async function fetchRepoMilestones(owner, repo, fetchImpl, token = null, opts: any = {}) {
  const ctx = normalizeApiCtx(opts?.ctx);
  const perPage = 100;
  const maxPages = Math.max(1, Math.min(10, Number(opts.maxPages) || 5));
 ...
 */
async function fetchRepoTags(owner, repo, fetchImpl, token = null, opts: any = {}) {
  const ctx = normalizeApiCtx(opts?.ctx);
  const perPage = 100;
 ...
 */
async function resolvePullRequestNodeId(owner: any, repo: any, pullNumber: any, fetchImpl: any, token: any, nodeId: any = null, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
 ...
 */
function mapViewerSubscription(state: any) {
...
async function deleteIssueSubscription(owner: any, repo: any, issueNumber: any, fetchImpl: any, token: any, nodeId: any = null) {
 ...
 */
async function fetchPullRequestSubscription(owner: any, repo: any, pullNumber: any, fetchImpl: any, token: any, nodeId: any = null, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  if (!token) return null;
  try {
    const id = await resolvePullRequestNodeId(
...
async function setIssueMilestone(owner: any, repo: any, issueNumber: any, milestoneNumber: any, fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  return apiSend(
 ...
 */
async function setPullRequestDraftStage(owner: any, repo: any, pullNumber: any, stage: any, fetchImpl: any, token: any, nodeId: any = null, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  let id = nodeId;
  if (!id) {
    const pr = await apiJson(
 ...
 */
async function fetchViewerLogin(fetchImpl: any, token: any, ctx: any = null) {
  ctx = normalizeApiCtx(ctx);
  if (!token) return null;
  try {
 ...
 */
function mapAndAnnotateFiles(files: any, gitattributesText: any = '') {
 ...
 */
async function fetchCompareFiles(owner, repo, base, head, fetchImpl, token = null, options: any = {}) {
  const ctx = normalizeApiCtx(options?.ctx);
  const o = String(owner || '').trim();
  const r = String(repo || '').trim();
