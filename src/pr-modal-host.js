...
function mergeDetailProgressive(prev, next, opts = null) {
  ...
  if (api && typeof api.mergeDetailProgressive === "function") {
    return api.mergeDetailProgressive(prev, next, opts || void 0);
  }
...
function ensureDetailStore(seedFlat = null) {
  ...
  if (!current.detailStore) {
    current.detailStore = seedFlat ? S.fromAppDetail(seedFlat) : S.createEmptyStore();
  }
...
function setDetailProgressive(next, opts = null) {
  ...
  if (S && current.detailStore) {
    ...
    if (Array.isArray(next.reviewThreads) && next.reviewThreads.length || Array.isArray(next.reviewComments) && next.reviewComments.length) {
  ...
  if (!S) {
    if (current.detail && mergedFlat) {
      current.detail = {
        ...
        viewerPendingReview: mergedFlat.viewerPendingReview !== void 0 ? mergedFlat.viewerPendingReview : current.detail.viewerPendingReview
      };
      return current.detail;
    }
...
function beginFetchTimeline(label) {
  ...
  const tl = {
    ...
    mark(name, phase, extra = null) {
      ...
      events.push(e);
      const dur = e.dur != null ? ` ${e.dur}ms` : e.phase === "end" && e.dur != null ? ` ${e.dur}ms` : "";
      const ok = e.ok === false ? " FAIL" : e.ok === true && e.phase === "end" ? " ok" : "";
      console.log(
        `[pr-plus][tl +${e.t}ms] ${e.phase} ${e.name}${dur}${ok}` + (e.err ? ` err=${e.err}` : "") + (e.note ? ` ${e.note}` : "")
      );
    ...
    span(name, promise, meta = null) {
      const tStart = nowMs();
      tl.mark(name, "start", meta || void 0);
...
function applyRouteFieldsFromTarget(target) {
  ...
  current.routeStartLine = target.startLine != null && Number.isFinite(Number(target.startLine)) ? Number(target.startLine) : null;
  current.routeEndLine = target.endLine != null && Number.isFinite(Number(target.endLine)) ? Number(target.endLine) : null;
...
function restoreGithubFooter() {
  ...
  try {
    document.querySelectorAll('[data-prp-footer-hidden="1"]').forEach((el) => {
...
function findGithubPrHeaderMount(doc = document) {
  ...
  try {
    const h1 = doc.querySelector(".js-issue-title") || doc.querySelector("h1.gh-header-title") || doc.querySelector('[data-testid="issue-title"]');
...
function ensureNativePrOpenShortcut() {
  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      const pe = pageEmbedApi();
      const resolve = pe?.resolveEmbedShortcutAction || typeof globalThis !== "undefined" && globalThis.PRModalPageEmbed?.resolveEmbedShortcutAction;
      const action = typeof resolve === "function" ? resolve({
...
function ensurePrefsWatch() {
  ...
  try {
    prefsWatchUnsub = globalThis.PRTreeStorage?.watchExtensionPrefs?.((next) => {
...
function fetchUnitWeights() {
  const lp = globalThis.PRModalLoadProgress;
  return lp && lp.FETCH_UNIT_WEIGHTS || {
    start: 4,
    core: 18,
    threadsNewest: 14,
    threadsFollow: 6,
...
function openProgressKeys() {
  const lp = globalThis.PRModalLoadProgress;
  return lp && lp.OPEN_PROGRESS_KEYS || [
    "start",
    "core",
    "threadsNewest",
    "threadsFollow",
...
function tryFinishOpenProgress(prog = activeOpenProgress) {
  ...
  const threadsOk = has("threadsVisible") || has("threadsNewest") && has("threadsFollow");
  const refreshDone = has("start") && has("core") && threadsOk && sidesDone;
...
function sideSettledFromDetail(detail) {
  ...
  const marked = d._sideSettled && typeof d._sideSettled === "object" ? d._sideSettled : {};
  const checks = d.checks;
  const hasCheckItems = checks && (Array.isArray(checks.statuses) && checks.statuses.length > 0 || Array.isArray(checks.checkRuns) && checks.checkRuns.length > 0 || Array.isArray(checks.check_runs) && checks.check_runs.length > 0);
  const fullSnap = d._cacheFull === true;
  return {
    commits: Boolean(marked.commits) || fullSnap || Array.isArray(d.commits) && d.commits.length > 0,
    checks: Boolean(marked.checks) || Boolean(hasCheckItems),
    development: Boolean(marked.development) || Array.isArray(d.developmentIssues) && d.developmentIssues.length > 0 || Array.isArray(d.linkedIssues) && d.linkedIssues.length > 0,
    files: Boolean(marked.files) || fullSnap || Array.isArray(d.files) && d.files.length > 0,
    comments: Boolean(marked.comments) || Array.isArray(d.comments) && d.comments.length > 0,
    reviews: Boolean(marked.reviews) || Array.isArray(d.reviews) && d.reviews.length > 0
  };
}
...
function kickIndependentSideFetches({
  ...
  if (!current._sideKickStarted || current._sideKickGen !== gen) {
    current._sideKickStarted = /* @__PURE__ */ new Set();
  ...
  const wrap = (name, p, meta = void 0) => tl && typeof tl.span === "function" ? tl.span(name, p, meta) : p;
  if (claim("files")) {
    markPendingIfNeeded("files");
    filesP = typeof api.fetchPrFiles === "function" ? wrap(
      "side.files",
      api.fetchPrFiles(owner, repo, number, {
        signal,
        headSha: headSha || null,
        gitattributesText: current.detail?.gitattributesText || ""
      }).then((pack) => {
        const files = Array.isArray(pack?.files) ? pack.files : Array.isArray(pack) ? pack : [];
        const gitattributesText = typeof pack?.gitattributesText === "string" ? pack.gitattributesText : current.detail?.gitattributesText || "";
  ...
  if (claim("comments")) {
    ...
    commentsP = typeof api.fetchPrIssueComments === "function" ? wrap(
      "side.comments",
      api.fetchPrIssueComments(owner, repo, number, { signal }).then((page) => {
  ...
  if (claim("reviews")) {
    ...
    reviewsP = typeof api.fetchPrReviews === "function" ? wrap(
      "side.reviews",
      api.fetchPrReviews(owner, repo, number, { signal }).then((reviews) => {
        settleSide("reviews", {
  ...
  if (claim("commits")) {
    ...
    commitsP = typeof api.fetchPrCommits === "function" ? wrap(
      "side.commits",
      api.fetchPrCommits(owner, repo, number, { signal }).then((commits) => {
        settleSide("commits", {
  ...
  if (headSha && claim("checks")) {
    ...
    checksP = typeof api.fetchPrChecks === "function" ? wrap(
      "side.checks",
      api.fetchPrChecks(owner, repo, headSha, { signal }).then((checks) => {
        settleSide("checks", {
          checks: checks || {
            state: "unknown",
            totalCount: 0,
  ...
  if (claim("development")) {
    ...
    developmentP = typeof api.fetchPrDevelopment === "function" ? wrap(
      "side.development",
      api.fetchPrDevelopment(owner, repo, number, {
        signal,
        body: body || ""
      }).then((dev) => {
        ...
        settleSide("development", {
          linkedIssues: Array.isArray(dev.linkedIssues) ? dev.linkedIssues : [],
          developmentIssues: Array.isArray(dev.developmentIssues) ? dev.developmentIssues : [],
...
function beginFetchProgress(gen, stillOpenFn = null) {
  ...
  const tracker = lp && typeof lp.createWeightProgress === "function" ? lp.createWeightProgress({ total: 100, initial: 0 }) : {
    complete(key, weight) {
      this._c = Math.min(100, (this._c || 0) + Math.max(0, weight || 0));
      this._keys = this._keys || /* @__PURE__ */ new Set();
    ...
    has(key) {
      return (this._keys || /* @__PURE__ */ new Set()).has(String(key || ""));
    },
    getKeys() {
      return [...this._keys || []];
    },
...
function setLoadStage(phase, label, busy = true, opts = null) {
  ...
  const fraction = opts && Number.isFinite(opts.phaseFraction) ? opts.phaseFraction : void 0;
  const percent = opts && Number.isFinite(opts.percent) ? Math.min(100, Math.max(0, Math.round(opts.percent))) : loadStagePercent(phase, b, fraction);
  const prev = current.loadStage && Number.isFinite(current.loadStage.percent) ? Number(current.loadStage.percent) : 0;
  const nextPercent = b && current.loadStage && current.loadStage.busy ? Math.max(prev, percent) : percent;
  current.loadStage = {
    phase: phase || null,
    label: label || null,
    busy: b,
...
function clearLoadStage() {
  try {
    ...
    if (tl) {
      ...
      if (ends.length) {
        console.log(
          "[pr-plus][tl] fetch durations: " + ends.map(
            (e) => `${e.name}=${e.dur != null ? e.dur + "ms" : "?"}${e.ok === false ? "!" : ""}`
          ).join(" ")
        );
      }
    }
...
const detailCache = globalThis.PRModalDetailCache?.createPersistedDetailCache?.({
  ttlMs: 6e4,
  createIdb: () => globalThis.PRModalDetailIdb?.createDetailIdb?.({
    maxEntries: 24,
    maxAgeMs: 7 * 24 * 60 * 60 * 1e3
  }) || null
}) || globalThis.PRModalDetailCache?.createDetailCache?.({ ttlMs: 6e4 }) || createFallbackCache();
function sessionApi() {
...
function ensureOpenPullsForStack(owner, repo, opts = {}) {
  ...
  if (openPullsFetchedKey === key && Array.isArray(openPullsFetched) && openPullsFetched.length) {
  ...
  openPullsFetchP = (async () => {
    try {
      ...
      if (Array.isArray(prs) && prs.length) {
        ...
        if (current.open && String(current.owner || "").toLowerCase() === o.toLowerCase() && String(current.repo || "").toLowerCase() === r.toLowerCase()) {
          render();
        }
        return prs;
      }
...
function detailSketchFromList(listPr, owner, repo, number) {
  const n = Number(number);
  if (!Number.isFinite(n) || n <= 0) return null;
  const title = listPr && String(listPr.title || "").trim() || `Pull Request #${n}`;
  const body = listPr && listPr.body != null ? String(listPr.body) : "";
  const author = listPr && listPr.author || "";
  const labels = Array.isArray(listPr?.labels) ? listPr.labels.map(
    (l) => typeof l === "string" ? { name: l, color: "", description: "" } : {
      name: l?.name || "",
      color: l?.color || "",
      description: l?.description || ""
    }
  ).filter((l) => l.name) : [];
  const assignees = Array.isArray(listPr?.assignees) ? listPr.assignees.map((u) => typeof u === "string" ? u : u?.login || "").filter(Boolean) : [];
  const requestedReviewers = Array.isArray(listPr?.requestedReviewers) ? listPr.requestedReviewers.map((u) => typeof u === "string" ? u : u?.login || "").filter(Boolean) : [];
  let milestone = null;
  ...
  const avatarUrls = listPr?.avatarUrls && typeof listPr.avatarUrls === "object" ? { ...listPr.avatarUrls } : {};
  ...
  return {
    ...
    htmlUrl: listPr?.htmlUrl || `https://github.com/${owner}/${repo}/pull/${n}`,
    merged: false,
    mergeable: null,
    labels,
    assignees,
...
function detailRank(d) {
  ...
  const hasThreads = Array.isArray(d.reviewComments) && d.reviewComments.length > 0 || Array.isArray(d.reviewThreads) && d.reviewThreads.length > 0;
...
function buildProps() {
  ...
  const presentation = isEmbedPresentation(current.presentation) ? "embed" : "modal";
  const chrome = presentation === "embed" && pageEmbedApi()?.embedShellChromeFlags ? pageEmbedApi().embedShellChromeFlags() : {
    presentation: "modal",
    showClose: true,
  ...
  return {
    ...
    onClose: presentation === "embed" ? () => {
    } : closeModal,
    onRestoreNative: presentation === "embed" ? () => restoreNativeView() : void 0,
    /**
    ...
    onOpenStackPr: (n, opts = {}) => {
      ...
      const page = opts?.page === "diff" || opts?.page === "conversation" ? opts.page : current.routePage === "diff" || current.routePage === "conversation" ? current.routePage : null;
      void openModal({
        owner,
        repo,
        number: Number(n),
    ...
    onRefresh: async (opts = {}) => {
      ...
      const mode = modeRaw === "full-threads" ? "full-threads" : modeRaw === "visible-threads" ? "visible-threads" : "revalidate";
      const visibleIds = [
        ...new Set(
          (Array.isArray(opts?.threadNodeIds) ? opts.threadNodeIds : []).map((id) => String(id || "").trim()).filter(Boolean)
        )
      ];
      ...
      setLoadStage(
        "refresh",
        mode === "full-threads" ? loadStageLabel("refresh-all") : mode === "visible-threads" ? loadStageLabel("refresh-visible") : loadStageLabel("refresh"),
        true
      );
      render();
      const stillOpen = () => gen === detailFetchGen && !signal.aborted && current.open && current.owner === owner && current.repo === repo && Number(current.number) === Number(number);
      const mergeFn = globalThis.PRTreeFetch.mergeReviewThreadsPageIntoDetail || null;
      const apiMax = 100;
      const nowMs = () => typeof performance !== "undefined" && performance.now ? performance.now() : Date.now();
      try {
        let paintRefreshCore2 = function(raw) {
          if (!stillOpen() || !raw) return null;
          let detail2 = raw;
          const threadSrc = current.detail && Array.isArray(current.detail.reviewComments) && current.detail.reviewComments.length ? current.detail : prevDetail;
          if (threadSrc && Array.isArray(threadSrc.reviewComments) && threadSrc.reviewComments.length && (!Array.isArray(detail2.reviewComments) || !detail2.reviewComments.length)) {
            detail2 = {
              ...detail2,
              reviewComments: threadSrc.reviewComments,
              reviewThreads: threadSrc.reviewThreads || detail2.reviewThreads,
              reviewThreadsMeta: threadSrc.reviewThreadsMeta || detail2.reviewThreadsMeta,
              reviewCommentsMeta: threadSrc.reviewCommentsMeta || detail2.reviewCommentsMeta
            };
          }
          if (prevDetail && Array.isArray(prevDetail.files) && prevDetail.files.length) {
            const netFiles = Array.isArray(detail2.files) ? detail2.files : [];
            const cachedHasPatches = prevDetail.files.some(
              (f) => f && typeof f.patch === "string" && f.patch.length > 0 && !f._patchOmitted
            );
            const netHasPatches = netFiles.some(
              (f) => f && typeof f.patch === "string" && f.patch.length > 0 && !f._patchOmitted
            );
            if (cachedHasPatches && !netHasPatches) {
              detail2 = { ...detail2, files: prevDetail.files };
            }
          }
          if (prevDetail && Array.isArray(prevDetail.commits) && prevDetail.commits.length && (!Array.isArray(detail2.commits) || !detail2.commits.length)) {
            detail2 = { ...detail2, commits: prevDetail.commits };
          }
          current.loading = false;
          ensureDetailStore(current.detail || prevDetail);
          applyCoreToStore(detail2);
          current.error = null;
          ...
          if (earlyRefreshThreadsPage) {
            paintRefreshThreadsNewest2(earlyRefreshThreadsPage);
          }
          if (earlyRefreshVisibleBulk) {
            paintRefreshVisibleBulk2(earlyRefreshVisibleBulk);
          }
          return current.detail;
        }, paintRefreshThreadsNewest2 = function(page) {
          if (!stillOpen() || !page || typeof mergeFn !== "function") return false;
          if (!current.detail) return false;
          const next2 = mergeFn(current.detail, page, "newest");
          applyThreadsToStore(next2);
          detailCache.set(key, current.detail);
          setLoadStage(
            "threads",
          ...
          render();
          return true;
        }, paintRefreshVisibleBulk2 = function(bulk) {
          if (!stillOpen() || !bulk || typeof mergeFn !== "function") return false;
          if (!current.detail) return false;
          const next2 = mergeFn(current.detail, bulk, "refresh");
          applyThreadsToStore(next2);
          detailCache.set(key, current.detail);
          setLoadStage(
            "threads",
        ...
        var paintRefreshCore = paintRefreshCore2, paintRefreshThreadsNewest = paintRefreshThreadsNewest2, paintRefreshVisibleBulk = paintRefreshVisibleBulk2;
        ...
        const threadsNewestP = mode !== "visible-threads" && canPageThreads ? globalThis.PRTreeFetch.fetchReviewThreadsPage(owner, repo, number, {
          direction: "newest",
          cursor: null,
          pageSize: apiMax,
          signal
          ...
          paintRefreshThreadsNewest2(page);
          return { ok: true, page };
        }).catch((err) => {
          prog.mark(
            "threadsNewest",
        ...
        const threadsVisibleP = mode === "visible-threads" && visibleIds.length && canBulkThreads ? globalThis.PRTreeFetch.fetchReviewThreadsByIds(visibleIds, { signal }).then((bulk) => {
          prog.mark(
            "threadsVisible",
            uw.threadsVisible,
            "threads",
        ...
        if (mode === "visible-threads") {
          if (visibleIds.length && canBulkThreads && typeof mergeFn === "function") {
            setLoadStage(
              "threads",
              loadStageLabel("threads-visible", { count: visibleIds.length }),
              true,
            ...
            console.log(
              `[pr-plus] onRefresh visible-threads ${owner}/${repo}#${number}: ${Math.round(
                nowMs() - tBulk
              )}ms (${bulk?.threads?.length || 0}/${visibleIds.length}` + (missingN ? `, dropped ${missingN} remote-missing` : "") + `, parallel-kickoff) pct=${prog.percent()}`
            );
            if (!stillOpen()) return;
            if (bulk) {
              const next2 = mergeFn(current.detail, bulk, "refresh");
        ...
        let next = typeof mergeFn === "function" ? mergeFn(current.detail, newest, "newest") : current.detail;
        applyThreadsToStore(next);
        next = current.detail;
        detailCache.set(key, next);
        render();
        const updatedIdSet = new Set(
          (newest?.threads || []).map((t) => t?.threadNodeId ? String(t.threadNodeId) : "").filter(Boolean)
        );
        const totalCount = typeof newest.totalCount === "number" ? newest.totalCount : newest.threads?.length || 0;
        if (mode === "full-threads") {
          ...
          const collectIds = globalThis.PRTreeFetch.collectUnresolvedThreadNodeIds || ((d) => {
          ...
          while (unresolvedPass < 2 && typeof globalThis.PRTreeFetch.fetchReviewThreadsByIds === "function" && typeof mergeFn === "function") {
            unresolvedPass += 1;
            ...
            const missingList = Array.isArray(bulk?.missingThreadIds) ? bulk.missingThreadIds : [];
            ...
            console.log(
              `[pr-plus] onRefresh unresolved-remaining ${owner}/${repo}#${number}: ${Math.round(
                nowMs() - tBulk
              )}ms (${bulk?.threads?.length || 0}/${remainingUnresolvedIds.length}` + (missingN ? `, dropped ${missingN} remote-missing` : "") + `, pass ${unresolvedPass})`
            );
            if (!stillOpen()) return;
            if (bulk) {
    ...
    onLoadMoreReviewThreads: async (direction) => {
      ...
      const loadAll = direction === "all" || direction === true || direction && String(direction).toLowerCase() === "all";
      ...
      const cursorFor = (meta, dir) => dir === "older" || dir === "newest" ? meta.newestStartCursor || meta.endCursor || null : meta.oldestEndCursor || null;
      const loadOnePage = async (detailSnap) => {
        ...
        if ((dir === "older" || dir === "newest") && !meta.hasOlder && meta.hasNewerFromOldest) {
          dir = "newer";
        }
        if ((dir === "newer" || dir === "oldest") && !meta.hasNewerFromOldest && meta.hasOlder) {
          dir = "older";
        }
        ...
        const afterCount = Number(next?.reviewThreadsMeta?.loadedThreadCount) || 0;
        return {
          detail: next,
      ...
      setLoadStage(
        "threads",
        loadAll ? loadStageLabel("threads-all", {
    ...
    onPatchDetail: (patch) => {
      ...
      if (owner && repo && number && (current.owner !== owner || current.repo !== repo || Number(current.number) !== Number(number))) {
        return;
      }
      detailFetchGen += 1;
      const next = {
        ...
        avatarUrls: {
          ...
          ...patch.avatarUrls && typeof patch.avatarUrls === "object" ? patch.avatarUrls : {}
        },
      ...
      if (Object.prototype.hasOwnProperty.call(patch, "requestedReviewers")) {
        next.requestedReviewers = Array.isArray(patch.requestedReviewers) ? patch.requestedReviewers : [];
      }
    ...
    onFetchCompareFiles: async (base, head, options = {}) => {
      ...
      return globalThis.PRTreeFetch.fetchCompareFiles(owner, repo, base, head, {
        gitattributesText: options.gitattributesText || current.detail?.gitattributesText || "",
        signal: openFetchAbort?.signal || null
      });
    },
...
function writeUriRoute(route = {}) {
  ...
  const position = route.position !== void 0 ? route.position : current.routePosition;
  const gh = githubRouteApi();
  const useGithubPath = isEmbedPresentation(current.presentation) && current.owner && current.repo && number != null && typeof gh?.replaceGithubPrLocation === "function";
  if (useGithubPath) {
    try {
      const commitSha = route.commitSha !== void 0 ? route.commitSha : current.routeCommitSha;
      const commitEndSha = route.commitEndSha !== void 0 ? route.commitEndSha : current.routeCommitEndSha;
      const filePath = route.filePath !== void 0 ? route.filePath : current.routeFilePath;
      const fileKey = route.fileKey !== void 0 ? route.fileKey : current.routeFileKey;
      const startLine = route.startLine !== void 0 ? route.startLine : current.routeStartLine;
      const endLine = route.endLine !== void 0 ? route.endLine : current.routeEndLine;
      const side = route.side !== void 0 ? route.side : current.routeSide;
...
function persistRouteState(route = {}) {
  ...
  if (route.position !== void 0) current.routePosition = route.position || null;
  if (route.commitSha !== void 0) {
    current.routeCommitSha = route.commitSha || null;
  }
  if (route.commitEndSha !== void 0) {
    current.routeCommitEndSha = route.commitEndSha || null;
  }
  if (route.filePath !== void 0) current.routeFilePath = route.filePath || null;
  if (route.fileKey !== void 0) current.routeFileKey = route.fileKey || null;
  if (route.startLine !== void 0) current.routeStartLine = route.startLine ?? null;
  if (route.endLine !== void 0) current.routeEndLine = route.endLine ?? null;
  if (route.side !== void 0) current.routeSide = route.side || null;
...
function abortOpenFetches(reason = "sheet-closed") {
  ...
  openFetchAbort = null;
  try {
    const ids = ac?.signal?.__prpRequestIds ? [...ac.signal.__prpRequestIds] : [];
...
function openModal({
  ...
  } else if (pathTarget && String(pathTarget.owner).toLowerCase() === String(owner).toLowerCase() && String(pathTarget.repo).toLowerCase() === String(repo).toLowerCase() && Number(pathTarget.number) === Number(number)) {
    resolvedPresentation = "embed";
  } else if (current.open && isEmbedPresentation(current.presentation) && String(current.owner).toLowerCase() === String(owner).toLowerCase() && String(current.repo).toLowerCase() === String(repo).toLowerCase() && Number(current.number) === Number(number)) {
    resolvedPresentation = "embed";
  }
  if (current.open && isEmbedPresentation(current.presentation) !== isEmbedPresentation(resolvedPresentation)) {
    dropReactRoot();
  ...
  const ghLoc = isEmbedPresentation(resolvedPresentation) || pathTarget && String(pathTarget.owner).toLowerCase() === String(owner).toLowerCase() && String(pathTarget.repo).toLowerCase() === String(repo).toLowerCase() && Number(pathTarget.number) === Number(number) ? parseGithubLocation() : null;
  const resolvedPage = page === "diff" || page === "conversation" ? page : pathTarget && isEmbedPresentation(resolvedPresentation) && (pathTarget.page === "diff" || pathTarget.page === "conversation") ? pathTarget.page : current.open && (current.routePage === "diff" || current.routePage === "conversation") ? current.routePage : page || null;
  const resolvedCommitSha = commitSha != null ? commitSha : ghLoc?.commitSha != null ? ghLoc.commitSha : pathTarget?.commitSha != null ? pathTarget.commitSha : null;
  const resolvedCommitEndSha = commitEndSha != null ? commitEndSha : ghLoc?.commitEndSha != null ? ghLoc.commitEndSha : pathTarget?.commitEndSha != null ? pathTarget.commitEndSha : null;
  ...
  const resolvedStartLine = startLine != null ? startLine : ghLoc?.startLine ?? null;
  ...
  const fetchTl = beginFetchTimeline(
    `open ${owner}/${repo}#${number}` + (fromCache ? " cache" : fromList ? " list" : " cold")
  );
  fetchTl.mark("first-paint-source", "mark", {
    note: fromCache ? `cache:${peeked?.source || "memory"}` : fromList ? "list-sketch" : "empty",
  ...
  current = {
    ...
    loadStage: {
      ...
      label: fromCache ? loadStageLabel("revalidate") : fromList ? loadStageLabel("core-full") : loadStageLabel("core"),
      busy: true,
  ...
  if (fromCache) {
    console.log(
      `[pr-plus] openModal cache-hit ${owner}/${repo}#${number} source=${peeked.source || "memory"} fresh=${Boolean(peeked.fresh)}`
    );
  } else if (fromList) {
    console.log(
      `[pr-plus] openModal list-sketch ${owner}/${repo}#${number} title=${JSON.stringify(String(listSketch.title || "").slice(0, 60))}`
    );
  }
  const idbHydrateP = !fromCache ? peekDetailIdb(key, 400).then((idbPeek) => {
    if (gen !== detailFetchGen) return null;
    if (!(current.open && current.owner === owner && current.repo === repo && Number(current.number) === Number(number))) {
      return null;
    }
    ...
    if (idbRank > curRank || current.detail?._sketch && idbRank >= 2) {
      ...
      render();
      console.log(
        `[pr-plus] openModal cache-hit ${owner}/${repo}#${number} source=idb (upgraded from ${curRank <= 1 ? "list/empty" : "partial"})`
      );
    }
    return v;
  ...
  return (async () => {
    try {
      let isAbortErr2 = function(err) {
        return err?.name === "AbortError" || /aborted|AbortError/i.test(String(err?.message || err || ""));
      }, mergeCoreWithCache2 = function(raw, cacheSnap) {
        let detail2 = raw;
        if (cacheSnap && Array.isArray(cacheSnap.reviewComments) && cacheSnap.reviewComments.length && (!Array.isArray(detail2.reviewComments) || !detail2.reviewComments.length)) {
          detail2 = {
            ...detail2,
            reviewComments: cacheSnap.reviewComments,
            reviewThreads: cacheSnap.reviewThreads || detail2.reviewThreads,
            reviewThreadsMeta: cacheSnap.reviewThreadsMeta || detail2.reviewThreadsMeta,
            reviewCommentsMeta: cacheSnap.reviewCommentsMeta || detail2.reviewCommentsMeta,
            comments: Array.isArray(detail2.comments) && detail2.comments.length ? detail2.comments : cacheSnap.comments || detail2.comments
          };
        }
        if (cacheSnap && Array.isArray(cacheSnap.files) && cacheSnap.files.length) {
          ...
          if (cachedHasPatches && !netHasPatches) {
            detail2 = { ...detail2, files: cacheSnap.files };
          }
        }
        if (cacheSnap && Array.isArray(cacheSnap.commits) && cacheSnap.commits.length && (!Array.isArray(detail2.commits) || !detail2.commits.length)) {
          detail2 = { ...detail2, commits: cacheSnap.commits };
        }
        if (detail2 && typeof detail2 === "object") {
          detail2 = { ...detail2, _sketch: void 0, _source: "network" };
        }
        return detail2;
      }, paintCoreNow2 = function(raw) {
        if (!openStill() || !raw) return null;
        const fromNetwork = mergeCoreWithCache2(raw, cached);
        ensureDetailStore(current.detail);
        applyCoreToStore(fromNetwork);
        current.loading = false;
        current.error = null;
        const detail2 = current.detail;
        detailCache.set(key, detail2);
        corePainted = true;
        setLoadStage(
          "threads",
          fromCache || detailRank(cached) >= 3 ? loadStageLabel("threads-update") : loadStageLabel("threads-load"),
          true,
          { percent: prog.percent() }
        );
        render();
        console.log(
          `[pr-plus] openModal phase=core-paint ${owner}/${repo}#${number} (prior=${fromCache ? "cache" : fromList ? "list" : "empty"}) pct=${prog.percent()}`
        );
        kickIndependentSideFetches({
          owner,
          repo,
          number,
          headSha: detail2.headSha || null,
          body: detail2.body || "",
          gen,
        ...
        if (earlyThreadsPage && typeof mergeFn === "function") {
          paintThreadsNewestNow2(earlyThreadsPage);
        }
        return detail2;
      }, paintThreadsNewestNow2 = function(page2) {
        if (!openStill() || !page2 || typeof mergeFn !== "function") return false;
        const base = current.detail;
        if (!base || typeof base !== "object") return false;
        const next = mergeFn(base, page2, "newest");
        ...
        render();
        console.log(
          `[pr-plus] openModal phase=threads.last-early-paint ${owner}/${repo}#${number} (${page2?.threads?.length || 0} threads) pct=${prog.percent()}`
        );
        return true;
      };
      var isAbortErr = isAbortErr2, mergeCoreWithCache = mergeCoreWithCache2, paintCoreNow = paintCoreNow2, paintThreadsNewestNow = paintThreadsNewestNow2;
      ...
      async function fetchDetailOnce(opts) {
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
          ...
          try {
            ...
            if (isAbortErr2(err)) throw err;
            lastErr = err;
            const msg = String(err?.message || err || "");
            if (/Extension context invalidated|Extension was reloaded/i.test(msg)) {
              throw err;
            }
            if (attempt === 0 && /message channel closed|Receiving end does not exist|Background worker offline/i.test(
              msg
            )) {
      ...
      const openStill = () => gen === detailFetchGen && current.open && current.owner === owner && current.repo === repo && Number(current.number) === Number(number);
      const prog = beginFetchProgress(gen, openStill);
      const uw = prog.weights;
      const corePhase = fromCache ? "revalidate" : "core";
      const coreLabel = fromCache ? loadStageLabel("revalidate") : fromList ? loadStageLabel("core-full") : loadStageLabel("core");
      prog.mark("start", uw.start, corePhase, coreLabel);
      const tCore0 = typeof performance !== "undefined" && performance.now ? performance.now() : Date.now();
      void idbHydrateP;
      const apiMax = 100;
      ...
      const tl = getFetchTimeline();
      const span = tl && typeof tl.span === "function" ? (name, p, meta = void 0) => tl.span(name, p, meta) : (_n, p, _m = void 0) => p;
      const threadsKickoffP = canFetchThreads ? span(
        "fetch.threadsNewest",
        globalThis.PRTreeFetch.fetchReviewThreadsPage(owner, repo, number, {
          ...
          earlyThreadsPage = page2;
          paintThreadsNewestNow2(page2);
          tl?.mark?.("paint.threadsNewest", "mark", {
            note: `${page2?.threads?.length || 0} threads`
          });
          return { ok: true, page: page2, paintedEarly: threadsPaintedEarly };
        }).catch((err) => {
          prog.mark(
            "threadsNewest",
            uw.threadsNewest,
      ...
      void idbHydrateP.then((idbVal) => {
        if (!openStill() || !idbVal) return;
        if (!cached) cached = idbVal;
        if (detailRank(cached) < detailRank(idbVal)) cached = idbVal;
        if (!corePainted && current.detail && (current.detail._sketch || detailRank(current.detail) < detailRank(idbVal))) {
          resetDetailStoreFromFlat(idbVal);
          current.loading = false;
          render();
          if (earlyThreadsPage) paintThreadsNewestNow2(earlyThreadsPage);
          kickIndependentSideFetches({
            owner,
            repo,
            number,
      ...
      const coreMs = Math.round(
        (typeof performance !== "undefined" && performance.now ? performance.now() : Date.now()) - tCore0
      );
      console.log(
        `[pr-plus] openModal phase=core ${owner}/${repo}#${number}: ${coreMs}ms ` + (detail?._fetchTimings ? JSON.stringify(detail._fetchTimings) : "(no per-request timings)") + ` pct=${prog.percent()} painted=${corePainted}`
      );
      if (!openStill()) return;
      if (!corePainted && detail) {
        detail = paintCoreNow2(detail) || detail;
      } else {
        detail = current.detail || detail;
      }
      if (canFetchThreads) {
        try {
          ...
          if (useRevalidatePath) {
            ...
            const knownMissing = /* @__PURE__ */ new Set();
            while (unresolvedPass < 2 && typeof globalThis.PRTreeFetch.fetchReviewThreadsByIds === "function") {
              unresolvedPass += 1;
              const remainingUnresolvedIds = collectIds(next).filter((id) => {
                const s = String(id);
                return !updatedIdSet.has(s) && !knownMissing.has(s);
              ...
              console.log(
                `[pr-plus] openModal phase=threads.unresolved-remaining ${owner}/${repo}#${number}: ${Math.round(
                  nowMs() - tBulk0
                )}ms (${bulk?.threads?.length || 0}/${remainingUnresolvedIds.length} ids, skipped last=${updatedIdSet.size}` + (missingN ? `, dropped ${missingN} remote-missing` : "") + `, pass ${unresolvedPass})`
              );
              if (gen !== detailFetchGen) return;
              if (current.open && Number(current.number) === Number(number) && typeof mergeFn === "function" && bulk) {
                next = mergeFn(next, bulk, "refresh");
              }
              if (!missingN) break;
            }
            prog.mark(
              ...
              uw.threadsFollow,
              "threads",
              didUnresolvedFetch ? loadStageLabel("threads-unresolved") : loadStageLabel("threads-update")
            );
            console.log(
            ...
            render();
            const totalCount = typeof newest.totalCount === "number" ? newest.totalCount : newest.threads?.length || 0;
            const needStartWindow = totalCount >= apiMax && Boolean(newest.hasPreviousPage);
            if (needStartWindow) {
              try {
                setLoadStage(
                  "threads",
            ...
            if (!fastReview && gen === detailFetchGen && current.open && current.detail?.reviewThreadsMeta?.hasMore) {
              try {
                const props = buildProps();
                if (typeof props.onLoadMoreReviewThreads === "function") {
                  await props.onLoadMoreReviewThreads("all");
      ...
      if (gen !== detailFetchGen || signal.aborted || err?.name === "AbortError" || /aborted|AbortError/i.test(String(err?.message || err || ""))) {
        return;
      }
      if (current.open) {
        current.loading = false;
...
async function tryRestoreOpenModal() {
  ...
  if (typeof bridge?.isExtensionContextAlive === "function" && !bridge.isExtensionContextAlive()) {
    return {
      ok: false,
      reason: "context-invalidated",
      message: bridge.RELOAD_REFRESH_MSG || "Extension was reloaded. Refresh this GitHub tab to reconnect pr+."
    };
  }
  ...
  if (sessionOpen && typeof sessionStorage !== "undefined" && sess?.loadSessionView) {
  ...
  const uriRoute = typeof uri?.parseLocationRoute === "function" ? uri.parseLocationRoute(typeof location !== "undefined" ? location : null) : { page: null, number: null, position: null };
  const resolved = typeof uri?.resolveRestore === "function" ? uri.resolveRestore({
    sessionOpen,
    sessionView,
    uri: uriRoute,
    pathOwner,
  ...
  if (resolved.source === "session") {
    if (pathOwner.toLowerCase() !== String(resolved.open.owner).toLowerCase() || pathRepo.toLowerCase() !== String(resolved.open.repo).toLowerCase()) {
...
function tryEmbedFromLocation() {
  ...
  const samePr = current.open && isEmbedPresentation(current.presentation) && String(current.owner).toLowerCase() === String(target.owner).toLowerCase() && String(current.repo).toLowerCase() === String(target.repo).toLowerCase() && Number(current.number) === Number(target.number);
  const sameSurface = samePr && current.routePage === target.page && String(current.routeCommitSha || "") === String(target.commitSha || "") && String(current.routeCommitEndSha || "") === String(target.commitEndSha || "") && String(current.routeFileKey || "") === String(target.fileKey || "") && Number(current.routeStartLine || 0) === Number(target.startLine || 0) && Number(current.routeEndLine || 0) === Number(target.endLine || 0);
  lastEmbedPath = locKey;
  if (sameSurface) {
    ensureEmbedHost();
    render();
...
function installEmbedWatch() {
  ...
  const onNav = () => {
    if (!hostEnabled) return;
    const locKey = embedLocationKey();
    if (locKey === lastEmbedPath && current.open && isEmbedPresentation(current.presentation)) {
      try {
        ensureEmbedHost();
        render();
      } catch {
  ...
  try {
    if (pollId != null && typeof pollId === "object" && typeof pollId.unref === "function") {
      pollId.unref();
    } else if (typeof pollId === "number" && typeof window.clearInterval === "function") {
    }
  } catch {
  }
}
function setEnabled(enabled) {
  ...
  installEmbedWatch();
  ensurePrefsWatch();
  void warmPrefs().catch(() => prefs).then(() => {
    tryEmbedFromLocation();
    try {
      ensureGithubPrToggle();
    } catch {
...
function applyPullsListFocus(index) {
  const api = listFocusApi();
  const rows = getPullsListRows();
  const row = api?.applyFocusToRows ? api.applyFocusToRows(rows, index) : (() => {
    for (const r2 of rows) {
      r2?.classList?.remove?.("prp-list-focus");
      try {
        r2?.removeAttribute?.("data-prp-list-focus");
      } catch {
      }
    }
...
function parsePrFromListRow(row) {
  ...
  const repo = typeof pathApi?.parseRepoFromPathname === "function" ? pathApi.parseRepoFromPathname(location.pathname || "") : null;
  if (!repo) {
...
function openNewPullRequestFromList() {
  ...
  const href = typeof api?.buildCreatePullRequestUrl === "function" ? api.buildCreatePullRequestUrl(
    repo.owner,
    repo.repo,
    getWebOrigin()
...
function findNewPullRequestControl() {
  try {
    ...
    for (const el of candidates) {
      const t = String(el.textContent || "").replace(/\s+/g, " ").trim().toLowerCase().replace(/⌥n/g, "").trim();
      if (t === "new pull request" || t === "new pr" || t.startsWith("new pull request")) {
        return el;
      }
      if (!fallback && el.matches?.('a[href*="/compare"]') && /compare/.test(el.getAttribute("href") || "")) {
        fallback = el;
      }
    }
    return fallback;
...
function findRowTitleAnchor(row) {
  ...
  return row.querySelector("a.js-navigation-open") || row.querySelector('a[id$="_link"]') || row.querySelector('h3 a[href*="/pull/"]') || row.querySelector('a[href*="/pull/"]');
}
function cleanControlLabel(el) {
  return String(el?.textContent || "").replace(/\s+/g, " ").replace(/⌥⇧?[A-Z0-9]/gi, "").replace(/alt\+shift\+[a-z]/gi, "").trim();
}
function findFilterBarControls() {
  const api = listFocusApi();
  const defs = api?.PR_LIST_FILTER_BAR || [];
  const matchFn = typeof api?.matchFilterBarLabel === "function" ? api.matchFilterBarLabel : null;
  const found = [];
  ...
  for (const scope of scopes) {
    ...
    for (const el of summaries) {
      ...
      const def = matchFn ? matchFn(text) : defs.find((d) => d.match?.test?.(text));
...
function setPullsListHotkeyHints(visible) {
  const api = listFocusApi();
  const slots = api && api.PR_LIST_HOTKEY_SLOTS || (typeof api?.buildPrListHotkeySlots === "function" ? api.buildPrListHotkeySlots() : "123456789abcdefgh ilmnopqrstuvwxyz".replace(/\s/g, ""));
  const attr = api && api.PR_LIST_HOTKEY_ATTR || "data-prp-list-hotkey";
  const newAttr = api && api.PR_LIST_NEW_PR_HOTKEY_ATTR || "data-prp-new-pr-hotkey";
  const filterAttr = api && api.PR_LIST_FILTER_BAR_ATTR || "data-prp-filter-bar-hotkey";
  const labelFn = typeof api?.prListHotkeyLabel === "function" ? api.prListHotkeyLabel : (i) => slots[i] || null;
  if (!visible) {
    try {
      document.documentElement.classList.remove("prp-opt-hints");
      document.querySelectorAll(`[${attr}], [${newAttr}], [${filterAttr}]`).forEach((el) => el.remove());
      document.querySelectorAll(".prp-filter-bar-float-host, .prp-filter-bar-host").forEach((el) => {
...
function stepPullsListFocus(delta) {
  ...
  const next = typeof api?.nextFocusIndex === "function" ? api.nextFocusIndex(cur, delta, rows.length) : (() => {
...
function rebuildPullsPaletteItems() {
  ...
  } else if (pullsPaletteFocusIndex < 0 || pullsPaletteFocusIndex >= n) {
...
function renderPullsPalettePrBody(item) {
  ...
  const numHtml = Number.isFinite(num) ? `<span class="prp-pp-pr-num">#${num}</span>` : "";
  const author = String(item?.author || "").trim();
  const authorHtml = author ? `<span class="prp-pp-author">${renderPullsPaletteAvatar(
    item
  ...
  let branchHtml = "";
  if (head || base) {
    const headChip = head ? `<span class="prp-pp-branch" title="${escapeHtml(head)}">${escapeHtml(
      head
    )}</span>` : "";
    const baseChip = base ? `<span class="prp-pp-branch prp-pp-branch--base" title="${escapeHtml(
      base
    )}">${escapeHtml(base)}</span>` : "";
    const arrow = head && base ? `<span class="prp-pp-branch-arrow" aria-hidden="true">\u2192</span>` : "";
    branchHtml = `<span class="prp-pp-branches">${headChip}${arrow}${baseChip}</span>`;
  }
  const draftHtml = item?.draft ? `<span class="prp-pp-draft">Draft</span>` : "";
  return `
...
function renderPullsPaletteActionBody(item) {
  const aliases = Array.isArray(item?.aliases) ? item.aliases.map((a) => String(a || "").trim()).filter(Boolean) : [];
  const aliasHtml = aliases.length ? `<span class="prp-pp-aliases">${aliases.map(
    (a) => `<kbd class="prp-pp-alias">${escapeHtml(a)}</kbd>`
  ).join("")}</span>` : "";
  const rawDesc = String(item?.description || item?.subtitle || "").trim();
  const aliasJoined = aliases.join(" \xB7 ");
  const desc = rawDesc && rawDesc !== aliasJoined && !(aliases.length === 1 && rawDesc === aliases[0]) ? rawDesc : "";
  let descShow = desc;
  ...
  const descHtml = descShow ? `<span class="prp-pp-action-desc">${escapeHtml(descShow)}</span>` : "";
  const meta = aliasHtml || descHtml ? `<span class="prp-pp-item__meta prp-pp-item__meta--action">${aliasHtml}${descHtml}</span>` : "";
  return `
      <span class="prp-pp-item__main">
        <span class="prp-pp-item__title">${escapeHtml(item?.title || "")}</span>
        ${meta}
...
function fillPullsPaletteHelp(root) {
  ...
  if (!list) return;
  const api = pullsPaletteApi();
  const entries = typeof api?.buildPullsPaletteHelpEntries === "function" ? api.buildPullsPaletteHelpEntries() : [];
  if (!entries.length) {
    list.innerHTML = '<div class="prp-pp-help__empty prp-muted">No actions configured</div>';
    return;
  }
  list.innerHTML = entries.map((e) => {
    const codes = (e.aliases || []).map(
      (a) => `<kbd class="prp-pp-help__alias">${escapeHtml(String(a))}</kbd>`
    ).join("");
    const fid = e.filterId ? ` data-prp-pp-help-filter="${escapeHtml(String(e.filterId))}"` : "";
...
function executePullsPaletteCommand(item) {
  ...
  if (item.action === "applyFilter" && item.filterId) {
    ...
    const href = typeof api?.buildPullsListFilterUrl === "function" ? api.buildPullsListFilterUrl(
      repo.owner,
      repo.repo,
      fid,
      getWebOrigin()
  ...
  if (item.action === "createPullRequest") {
    const repo = getRepoForPalette();
    const href = item.href || (typeof api?.buildCreatePullRequestUrl === "function" ? api.buildCreatePullRequestUrl(
      repo.owner,
      repo.repo,
      getWebOrigin()
    ) : `${getWebOrigin()}/${repo.owner}/${repo.repo}/compare`);
...
function runPullsPaletteHelpAction(el) {
  ...
  return executePullsPaletteCommand({
    ...
    kind: "action",
    action,
    filterId: filterId || void 0
  });
}
function togglePullsPaletteHelp(force = void 0) {
  ...
  if (open) {
    ...
    pullsPaletteRoot.querySelector(".prp-pp-panel")?.classList.add("prp-pp-panel--help");
  } else {
    panel.setAttribute("hidden", "");
    toggle?.setAttribute("aria-expanded", "false");
    pullsPaletteRoot.querySelector(".prp-pp-panel")?.classList.remove("prp-pp-panel--help");
  }
}
...
function paintPullsPalette() {
  ...
  if (meta) {
    const viewer = getViewerLoginForPalette();
    meta.textContent = viewer ? `Type to filter \xB7 open help for actions  \xB7  @${viewer}` : "Type to filter \xB7 open help for actions";
  }
  if (!listEl) return;
  if (items.length === 0) {
    listEl.removeAttribute("data-prp-pp-animate");
    listEl.innerHTML = '<li class="prp-pp-empty prp-muted">No matching results</li>';
    return;
  }
  listEl.setAttribute("data-prp-pp-animate", "1");
  listEl.innerHTML = items.map((item, i) => {
    const focused = i === pullsPaletteFocusIndex ? " is-focused" : "";
    const digit = item.digit != null ? `<kbd class="prp-pp-digit">\u2325${item.digit}</kbd>` : "";
    const isPr = item.kind === "pr";
    const body = isPr ? renderPullsPalettePrBody(item) : renderPullsPaletteActionBody(item);
    return `<li class="prp-pp-item${focused}${item.kind === "action" ? " prp-pp-item--action" : " prp-pp-item--pr"}" data-prp-pp-index="${i}" role="option" aria-selected="${i === pullsPaletteFocusIndex ? "true" : "false"}">
...
function closePullsPalette() {
  ...
  if (pullsPaletteSavedListFocus != null && Number.isFinite(Number(pullsPaletteSavedListFocus))) {
...
function openPullsPalette() {
        ...
        <div class="prp-pp-panel" role="dialog" aria-label="pr+ pulls command palette" aria-modal="true">
          <div class="prp-pp-main">
            ...
            <div class="prp-pp-foot">
              <span class="prp-pp-foot__keys prp-muted">\u2325\u21E7K \xB7 \u2191\u2193 \xB7 \u2325J \u2325K \xB7 Enter \xB7 Esc</span>
          ...
          <aside class="prp-pp-help" id="prp-pp-help-panel" data-prp-pp-help hidden>
            <div class="prp-pp-help__head">
              ...
              <button type="button" class="prp-pp-help-close" data-prp-pp-help-toggle aria-label="Close help">\xD7</button>
            </div>
            <div class="prp-pp-help__list" data-prp-pp-help-list></div>
            <div class="prp-pp-help__hint prp-muted">Click a row to run \xB7 or type alias + Enter</div>
          </aside>
        </div>`;
  ...
  pullsPaletteScrollbarDestroy = null;
  try {
    const attach = globalThis.PRModalFloatingScrollbar?.attachFloatingScrollbar;
...
function activatePullsPaletteItem(index) {
  ...
  const resolvedIdx = typeof api?.resolveActivateIndex === "function" ? api.resolveActivateIndex(items, index, pullsPaletteQuery) : index;
...
function stepPullsPaletteFocus(delta) {
  const api = pullsPaletteApi();
  const items = pullsPaletteItems || rebuildPullsPaletteItems() || [];
  const next = typeof api?.nextPaletteFocusIndex === "function" ? api.nextPaletteFocusIndex(
    pullsPaletteFocusIndex,
    delta,
    items.length
  ) : items.length ? (pullsPaletteFocusIndex + delta + items.length) % items.length : -1;
...
function onPullsListKeydown(event) {
  ...
  if (!isPullsPaletteOpen() && isPullsListPage() && alt && !mod && !shift && (event.code === "AltLeft" || event.code === "AltRight" || event.key === "Alt")) {
  ...
  if (typeof pp?.resolvePullsPaletteShortcutAction === "function") {
    ...
    const { action: action2, digitIndex } = typeof pp.unwrapPullsPaletteAction === "function" ? pp.unwrapPullsPaletteAction(raw) : { action: typeof raw === "string" ? raw : raw?.action, digitIndex: raw?.digitIndex ?? -1 };
    if (action2) {
      ...
      if (action2 === "openPalette") {
        openPullsPalette();
        return;
      }
      if (action2 === "closePalette") {
        closePullsPalette();
        return;
      }
      if (action2 === "focusNext") {
        stepPullsPaletteFocus(1);
        return;
      }
      if (action2 === "focusPrev") {
        stepPullsPaletteFocus(-1);
        return;
      }
      if (action2 === "activate") {
  ...
  const resolve = listApi?.resolvePrListShortcutAction || typeof globalThis !== "undefined" && globalThis.PRListFocus?.resolvePrListShortcutAction;
  ...
  const unwrapped = typeof listApi?.unwrapPrListAction === "function" ? listApi.unwrapPrListAction(rawList) : typeof rawList === "string" ? { action: rawList, index: -1, filterId: null } : {
...
function onClickCapture(event) {
  ...
  for (const n of nodes) {
    if (n && n.tagName === "A" && n.getAttribute?.("href")?.includes("/pull/")) {
      anchor = n;
      break;
    }
    if (n?.closest) {
  ...
  const looksLikeTitle = anchor.classList.contains("js-navigation-open") || anchor.classList.contains("markdown-title") || Boolean(anchor.id?.endsWith("_link")) || Boolean(inRow);
...
function listenClearDetailCache() {
  try {
    chrome.runtime?.onMessage?.addListener((message, _sender, sendResponse) => {
      ...
      void clearDetailCache().then((res) => {
        try {
