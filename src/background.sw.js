      ...
      if (commentsConnPresent)
        for (const node of t.comments?.nodes || []) {
          ...
          comments.push({
            ...
            isMinimized: node.isMinimized != null ? !!node.isMinimized : null,
            minimizedReason: node.minimizedReason ?? null,
            viewerCanMinimize: node.viewerCanMinimize != null ? !!node.viewerCanMinimize : null,
...
(function(global) {
  ...
  function formatStartReviewError(err) {
    ...
    return /locked/i.test(raw) || Number(err?.status) === 422 && /lock/i.test(raw) ? LOCKED_PR_START_REVIEW_MSG : raw;
  }
  ...
  function viewerHasPendingReview(input) {
    if (input == null || input === !1) return !1;
    if (input === !0) return !0;
    if (typeof input == "number")
      return Number.isFinite(input) && input > 0;
    if (typeof input == "object") {
      if (input.hasServerPending) return !0;
      const sid = input.serverPendingReviewId;
      return !!(sid != null && String(sid).trim() !== "" && String(sid) !== "0" || Number(input.pendingCount) > 0 || input.hasPendingReplies || input.rootPending);
    }
    return !!input;
  }
...
(function() {
  ...
  function formatCheckSummary(item, nowMs) {
    if (!item) return "";
    const desc = String(item.description || "").trim(), start = parseTime(item.startedAt), outcome = item.outcome || classifyCheckOutcome(item), live = outcome === "in_progress" || statusIsInProgress(item), now = Number.isFinite(nowMs) ? nowMs : Date.now();
    let duration = "";
    if (live && start)
      duration = formatDurationMs(Math.max(0, now - start));
    else {
      const end = parseTime(item.completedAt) || parseTime(item.updatedAt);
      start && end && end >= start ? duration = formatDurationMs(end - start) : start && !end && (duration = formatDurationMs(Math.max(0, now - start)));
    }
    if (outcome === "failure")
    ...
    return live ? duration ? `In progress \u2014 ${duration}` : desc || "In progress" : desc || "Expected \u2014 Waiting for status to be reported";
  }
  function checksNeedElapsedTick(checks) {
    const n = normalizeChecks(checks);
    for (const r of n.checkRuns || [])
      if (classifyCheckOutcome(r) === "in_progress" && parseTime(r.startedAt || r.started_at))
        return !0;
    for (const s of n.statuses || [])
      if (classifyCheckOutcome({ kind: "status", state: s.state }) === "in_progress" && parseTime(s.createdAt || s.created_at || s.startedAt || s.started_at))
        return !0;
    return !1;
  }
  ...
  function formatCheckGroupTip(outcome, names, locale) {
    const list = Array.isArray(names) ? names.filter(Boolean) : [], n = list.length, pure = typeof globalThis < "u" ? globalThis.PRModalI18n : null, t = (key, subs) => {
    ...
    }[outcome] || t("check_tip_checks_n", { count: n }) || `${n} checks`;
    if (!n) return head;
    const shown = list.slice(0, 12), more = n - shown.length, body = shown.map((name) => `\xB7 ${name}`).join(`
`), moreLine = t("check_tip_more", { count: more }) || `\xB7 +${more} more`;
    return more > 0 ? `${head}
${body}
${moreLine}` : `${head}
${body}`;
...
(function(global) {
  ...
  function mergeCommentMinimizeFields(prev, next) {
    if (!prev) return next;
    if (!next) return prev;
    const pickMin = (a, b) => a?.isMinimized != null ? !!a.isMinimized : b?.isMinimized != null ? !!b.isMinimized : !1, nextKnowsMin = next.isMinimized != null, isMinimized = nextKnowsMin ? !!next.isMinimized : pickMin(prev, next), minimizedReason = isMinimized ? nextKnowsMin ? next.minimizedReason ?? prev.minimizedReason ?? null : prev.minimizedReason ?? next.minimizedReason ?? null : null, viewerCanMinimize = next.viewerCanMinimize != null ? !!next.viewerCanMinimize : prev.viewerCanMinimize != null ? !!prev.viewerCanMinimize : null;
    return {
      ...prev,
      ...next,
      isMinimized,
  ...
  function mergeTimelineItemsById(prevItems, nextItems, opts = {}) {
    ...
    for (const it of Array.isArray(nextItems) ? nextItems : []) {
      if (!it) continue;
      const k2 = keyOf(it, i++), prev = map.get(k2);
      map.set(k2, prev ? mergeCommentMinimizeFields(prev, it) : it);
    }
  ...
  function isTimelineLoadIncomplete(meta) {
    if (!meta || typeof meta != "object") return !1;
    if (meta.hasMore || meta.complete === !1) return !0;
    if (meta.complete === !0) return !1;
    const total = Number(meta.totalCount), loaded = Number(meta.loadedCount);
    return !!(Number.isFinite(total) && Number.isFinite(loaded) && total > loaded);
  }
  ...
  function buildThreadEntry(c, children, snippetFn, files, viewerLogin, i) {
    const replies = (children.get(String(c.id)) || []).slice().sort(
      ...
      isMinimized: !!(r.isMinimized ?? r.is_minimized ?? !1),
      minimizedReason: r.minimizedReason ?? r.minimized_reason ?? null,
      viewerCanMinimize: r.viewerCanMinimize != null ? !!r.viewerCanMinimize : r.viewer_can_minimize != null ? !!r.viewer_can_minimize : null,
    ...
    return {
      ...
      isMinimized: !!(c.isMinimized ?? c.is_minimized ?? !1),
      minimizedReason: c.minimizedReason ?? c.minimized_reason ?? null,
      viewerCanMinimize: c.viewerCanMinimize != null ? !!c.viewerCanMinimize : c.viewer_can_minimize != null ? !!c.viewer_can_minimize : null,
...
function normalizeListOpenModePref(raw) {
  const v = String(raw ?? "").trim().toLowerCase();
  return v === "page" || v === "pr-page" || v === "pr_page" || v === "navigate" || v === "native" || v === "github" ? "page" : v === "modal" || v === "overlay" || v === "sheet" || v === "pr+" || v === "prp" || v === "pr-plus" ? "modal" : DEFAULT_PREFS.listOpenMode;
}
function normalizeUiLanguagePref(raw) {
  ...
  return lower === "auto" || lower === "detect" || lower === "default" || lower === "system" || lower === "github" ? "auto" : v === "zh_CN" || lower === "zh-cn" || lower === "zh_cn" || lower === "zh" ? "zh_CN" : lower === "en" || lower.startsWith("en-") ? "en" : lower === "ko" || lower.startsWith("ko-") ? "ko" : lower === "ja" || lower.startsWith("ja-") ? "ja" : DEFAULT_PREFS.uiLanguage;
}
...
async function setExtensionPrefs(patch, storageApi2) {
  const area = getStorageArea(storageApi2);
  if (!area) return Promise.reject(new Error("chrome.storage unavailable"));
  const prev = await getExtensionPrefs(area), patchObj = patch && typeof patch == "object" ? patch : {}, next = normalizePrefs({
    ...prev,
    ...patchObj
  });
  return Object.prototype.hasOwnProperty.call(patchObj, "uiLanguage") && (next.uiLanguage = normalizeUiLanguagePref(patchObj.uiLanguage)), new Promise((resolve, reject) => {
...
function mapIssueComment(c) {
  return {
    ...
    isMinimized: c.isMinimized != null || c.is_minimized != null ? !!(c.isMinimized ?? c.is_minimized) : null,
    minimizedReason: c.minimizedReason ?? c.minimized_reason ?? null,
    viewerCanMinimize: c.viewerCanMinimize != null ? !!c.viewerCanMinimize : c.viewer_can_minimize != null ? !!c.viewer_can_minimize : null
  };
}
function mapReviewComment(c, extra = {}) {
  ...
  return {
    ...
    isMinimized: extra.isMinimized != null || c.isMinimized != null || c.is_minimized != null ? !!(extra.isMinimized ?? c.isMinimized ?? c.is_minimized) : null,
    minimizedReason: extra.minimizedReason ?? c.minimizedReason ?? c.minimized_reason ?? null,
    viewerCanMinimize: extra.viewerCanMinimize != null ? !!extra.viewerCanMinimize : c.viewerCanMinimize != null ? !!c.viewerCanMinimize : null,
...
function mapGraphqlReviewCommentNode(node, threadMeta = {}) {
  ...
  return {
    ...
    isMinimized: !!(node.isMinimized ?? !1),
    minimizedReason: node.minimizedReason ?? null,
    viewerCanMinimize: node.viewerCanMinimize != null ? !!node.viewerCanMinimize : null,
...
async function fetchMinimizableStates(nodeIds, fetchImpl, token, ctx = null) {
  ...
  try {
    const data = await apiGraphql(query, { ids }, fetchImpl, token, ctx);
    for (const node of data?.nodes || [])
      node?.id && out.set(String(node.id), {
...
function applyMinimizableStates(items, byId) {
  ...
  for (const c of items) {
    ...
    m && (c.isMinimized = !!m.isMinimized, c.minimizedReason = m.minimizedReason ?? null, m.viewerCanMinimize != null && (c.viewerCanMinimize = !!m.viewerCanMinimize));
  }
  return items;
}
...
function isReactableGraphqlId(id) {
  ...
  return !s || /^\d+$/.test(s) || /^shell:/i.test(s) || /^rest-thread-/i.test(s) || /^PRRT_/i.test(s) || /PullRequestReviewThread/i.test(s) ? !1 : !!(/^(PRRC_|IC_|I_|PR_)/i.test(s) || /^[A-Za-z0-9_=-]{12,}$/.test(s) && /_/.test(s));
}
...
function restReactionCommentId(id) {
  ...
  const s = String(id).trim();
  return s ? /^\d+$/.test(s) ? s : isReactableGraphqlId(s) ? null : s : null;
}
async function toggleCommentReaction(owner, repo, kind, opts, fetchImpl, token, ctx = null) {
  ...
  const rawNodeId = String(opts?.nodeId || "").trim(), nodeId = isReactableGraphqlId(rawNodeId) ? rawNodeId : "", nextReacted = !opts?.viewerHasReacted, kRaw = String(kind || "issue").toLowerCase(), k = kRaw === "review" ? "review" : kRaw === "pr" ? "pr" : "issue";
  if (nodeId) {
    ...
    try {
      return await apiGraphql(
        ...
        token,
        ctx
      ), { content, reacted: nextReacted, via: "graphql" };
    } catch (err) {
      const restId = restReactionCommentId(k === "pr" ? opts?.number ?? opts?.commentId : opts?.commentId);
      if (!(isGraphqlNotFoundError(err) && restId != null)) {
        const msg = String(err?.message || err || "GraphQL reaction failed"), e = new Error(
          `${msg} | rxv4 via=graphql nodeId=${nodeId} kind=${k}`
        );
        throw e.status = err?.status ?? 200, e.via = "graphql", e.nodeId = nodeId, e.graphqlErrors = err?.graphqlErrors, e;
      }
    }
  }
  let basePath = "", deletePath = (reactionId) => "", restSubject = null;
  if (k === "review") {
    const commentId = restReactionCommentId(opts?.commentId);
    if (restSubject = commentId, commentId == null)
      throw new Error(
  ...
  if (nextReacted)
    try {
      ...
      ), { content, reacted: !0, via: "rest" };
    } catch (err) {
      const restMsg = String(err?.message || err || "REST reaction failed"), e = new Error(
        `${restMsg} | rxv4 via=rest subject=${restSubject} path=${basePath}`
      );
      throw e.status = err?.status ?? 403, e.via = "rest", e.restPath = basePath, e;
    }
  ...
  return target?.id ? (await apiSend(
    ...
    token,
    { method: "DELETE" }
  ), { content, reacted: !1, via: "rest" }) : { content, reacted: !1, via: "rest" };
}
...
async function fetchPrCommentsPage(owner, repo, pullNumber, kind, opts, fetchImpl, token, ctx = null) {
  ...
  async function fetchPage(pageNum, listOpts = {}, ctx2 = null) {
    ...
    if (token && items.length) {
      ...
      if (ids.length)
        try {
          const reactionsP = kind === "issue" ? fetchReactableReactionGroups(ids, fetchImpl, token, ctx2) : Promise.resolve(null), [byId, minById] = await Promise.all([
            reactionsP,
...
async function createPendingPullReview(owner, repo, pullNumber, opts = {}, fetchImpl, token, ctx = null) {
  ctx = normalizeApiCtx(ctx);
  const body = {};
  return opts?.commitId && (body.commit_id = String(opts.commitId)), apiSend(
...
async function resolveViewerPendingReviewViaGraphql(owner, repo, pullNumber, fetchImpl, token, ctx = null, opts = {}) {
  ...
  try {
    ...
    const login = opts.login != null ? String(opts.login).toLowerCase() : "", preferId = opts.preferDatabaseId != null && Number.isFinite(Number(opts.preferDatabaseId)) ? Number(opts.preferDatabaseId) : null;
    ...
    return !Number.isFinite(id) || id <= 0 ? null : { id, node_id: String(last.id) };
  } catch {
    return null;
  }
}
async function ensureViewerPendingReview(owner, repo, pullNumber, { commitId = null, createIfMissing = !1 } = {}, fetchImpl, token, ctx = null) {
  ...
  const hydrateNodeId = async (pending2) => {
    ...
    try {
      ...
      return !full || String(full.state || "").toUpperCase() !== "PENDING" ? pending2?.node_id ? { id: Number(pending2.id), node_id: pending2.node_id } : null : {
        id: Number(pending2.id),
        node_id: full.node_id || pending2.node_id || null
      };
    } catch (err) {
      return err?.status === 404 ? pending2?.node_id ? { id: Number(pending2.id), node_id: pending2.node_id } : null : pending2?.node_id ? { id: Number(pending2.id), node_id: pending2.node_id } : pending2?.id ? { id: Number(pending2.id), node_id: null } : null;
    }
  }, withGraphqlNode = async (pending2, opts = {}) => {
    if (pending2?.node_id) return pending2;
    const force = !!opts.forceDiscover;
    if (!pending2?.id && createIfMissing && !force) return pending2;
    const login = await fetchViewerLogin(fetchImpl, token).catch(() => null), viaGql = await resolveViewerPendingReviewViaGraphql(
      owner,
      repo,
      pullNumber,
      fetchImpl,
    ...
    return viaGql?.node_id ? viaGql : pending2;
  };
  ...
  if (pending = await hydrateNodeId(pending), pending = await withGraphqlNode(pending), pending?.node_id) return pending;
  if (!createIfMissing)
    return pending = await withGraphqlNode(pending, { forceDiscover: !0 }), pending;
  try {
    ...
    return createdRow.node_id ? createdRow : (pending = await hydrateNodeId(createdRow), pending = await withGraphqlNode(pending || createdRow, {
      forceDiscover: !0
    }), pending);
  } catch (err) {
    const msg = String(err?.message || err || "");
    if (err?.status === 422 || /one pending review/i.test(msg) || /Unprocessable Entity/i.test(msg)) {
      for (let attempt = 0; attempt < 5; attempt++)
        if (attempt > 0 && await new Promise((r) => setTimeout(r, 250 * attempt)), pending = await findViewerPendingReview(
          owner,
          repo,
          pullNumber,
          fetchImpl,
          token,
          ctx
        ), pending = await hydrateNodeId(pending), pending = await withGraphqlNode(pending, { forceDiscover: !0 }), pending?.node_id) return pending;
    }
    throw err;
  }
}
...
async function postReviewComment(owner, repo, pullNumber, {
  ...
  if (!pending?.node_id && asPending)
    try {
      pending = await ensureViewerPendingReview(
        ...
        repo,
        pullNumber,
        { commitId: commitId || null, createIfMissing: !0 },
        fetchImpl,
        token,
        ctx
      );
      ...
      if (ensureErr?.status === 422 || /one pending review/i.test(em) || /Unprocessable Entity/i.test(em)) {
        for (let i = 0; i < 6 && !pending?.node_id; i++)
          i > 0 && await new Promise((r) => setTimeout(r, 300 * i)), pending = await ensureViewerPendingReview(
            owner,
            repo,
            pullNumber,
            { commitId: commitId || null, createIfMissing: !1 },
  ...
  async function attachViaGraphql(pendingRow) {
    ...
    const nodeId = String(pendingRow.node_id), raw = await postReviewCommentViaPendingGraphql(
      nodeId,
      gqlFields,
      fetchImpl,
      token,
  ...
  async function recoverAttachFromExistingPending() {
    ...
    if (!row?.node_id && pendingReviewId && (row = await ensureViewerPendingReview(
      owner,
      repo,
      pullNumber,
      { commitId: commitId || null, createIfMissing: !1 },
  ...
  if (pending?.node_id)
    try {
      ...
      const msg = String(err?.message || err || "");
      if (asPending && (/Could not resolve to a node|global id|NOT_FOUND|Could not find/i.test(
        msg
      ) || /one pending review/i.test(msg) || err?.status === 422)) {
        const recovered = await recoverAttachFromExistingPending();
        if (recovered) return recovered;
        if (pending = await ensureViewerPendingReview(
          owner,
          repo,
          pullNumber,
          { commitId: commitId || null, createIfMissing: !1 },
  ...
  commitId && (payload.commit_id = commitId), !isFile && startLine != null && Number(startLine) !== Number(line) && (payload.start_line = Number(startLine), payload.start_side = startSide || side || "RIGHT");
  try {
    return await apiSend(
...
async function minimizeComment(subjectNodeId, classifier = "OFF_TOPIC", fetchImpl, token, ctx = null) {
  ...
  return MINIMIZE_CLASSIFIERS.includes(c) || (c = "OFF_TOPIC"), apiGraphql(
...
async function resolveReviewCommentNodeId(owner, repo, commentId, fetchImpl, token, ctx) {
  ...
  try {
    const revs = await apiJson(
      ...
      fetchImpl,
      token
    ), pending = (Array.isArray(revs) ? revs : []).filter(
    ...
    for (const r of pending)
      try {
        const comments = await apiJson(
          ...
          fetchImpl,
          token
        ), hit = (Array.isArray(comments) ? comments : []).find(
          (c) => Number(c?.id) === idNum
        );
...
async function editReviewComment(owner, repo, commentId, body, fetchImpl, token, ctx = null, opts = null) {
  ctx = normalizeApiCtx(ctx);
  const nextBody = String(body || ""), nodeIdRaw = opts?.nodeId != null && String(opts.nodeId).trim() ? String(opts.nodeId).trim() : "", preferGraphql = /^PRRC_/i.test(nodeIdRaw);
  opts?.pullNumber != null && Number.isFinite(Number(opts.pullNumber)) && (ctx = { ...ctx, pullNumber: Number(opts.pullNumber) });
  ...
  try {
    ...
    const status = Number(err?.status || err?.statusCode || 0), msg = String(err?.message || err || "");
    if (!(status === 404 || /\b404\b/.test(msg) || /not\s*found/i.test(msg))) throw err;
    let nodeId = nodeIdRaw;
    if (/^PRRC_/i.test(nodeId) || (nodeId = await resolveReviewCommentNodeId(
      owner,
      repo,
      commentId,
      fetchImpl,
...
function isUnverifiedLocalOnlyReviewCommentLocal(c) {
  if (!c || c.id == null || c.pending || c._commentsPending || c.commentsLoaded === !1 || String(c.id).startsWith("shell:") || String(c.body ?? c.bodyText ?? c.bodyHTML ?? "").trim()) return !1;
  const author = String(
    c.author || c.user?.login || c.user?.name || ""
  ).trim();
  return !(author && author.toLowerCase() !== "user");
}
...
function mergeReviewThreadsPageIntoDetail(detail, page, direction = "older") {
  ...
  if ((dir === "refresh" || dir === "ids") && (page?.threads || []).length) {
    ...
    if (page?.source === "graphql" && Array.isArray(page?.threads)) {
      ...
      pageThreadIds.size ? baseRc = baseRc.filter((c) => {
        if (!c) return !1;
        const tid = c.threadNodeId != null ? String(c.threadNodeId) : "";
        return !(tid.startsWith("rest-thread-") || pageThreadIds.has(tid) || deferredShellIds.has(tid) || isUnverifiedLocalOnlyReviewCommentLocal(c));
      }) : baseRc = baseRc.filter(
        (c) => c && !isUnverifiedLocalOnlyReviewCommentLocal(c)
      );
    }
    reviewComments = mergePendingReviewComments(baseRc, page?.comments || []), reviewComments = (Array.isArray(reviewComments) ? reviewComments : []).filter(
      (c) => c && !isUnverifiedLocalOnlyReviewCommentLocal(c)
    );
  }
  ...
  let stampedComments = stampedCommentsRaw.filter((c) => !c || isUnverifiedLocalOnlyReviewCommentLocal(c) ? !1 : c?._commentsPending ? !realCommentThreadIds.has(String(c.threadNodeId || "")) : !0);
  try {
