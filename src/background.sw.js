    ...
    for (const t of Array.isArray(allNodes) ? allNodes : []) {
      ...
      if (commentsConnPresent)
        for (const node of t.comments?.nodes || []) {
          ...
          commentIds.push(id), comments.push({
            id,
            body: node.body || "",
...
(function(global) {
  ...
  function buildConversationTimeline(detail, opts = {}) {
    ...
    for (const [rid, threads] of threadsByReviewId) {
      const review = reviewById.get(rid), reviewBody = String(review?.body || "").trim(), anyPending = threads.some((t) => t.pending) || String(review?.state || "").toUpperCase() === "PENDING" || rid === "viewer-pending" || viewerPendingId != null && rid === viewerPendingId, state = anyPending ? "PENDING" : String(review?.state || "COMMENTED").toUpperCase(), shouldGroup = anyPending || threads.length >= 2 || threads.length >= 1 && !!reviewBody || threads.length >= 1 && state && state !== "COMMENTED" && state !== "PENDING";
      if (threads.sort((a, b) => {
  ...
  function partitionTimelineWithThreadGap(items, meta = null) {
    const list = Array.isArray(items) ? items : [], hiddenCount = Math.max(0, Number(meta?.hiddenCount) || 0), oldestIds = new Set(
      (meta?.oldestThreadIds || []).map(String).filter(Boolean)
    ), wantsGap = !!meta?.hasMore && hiddenCount > 0;
  ...
  function timelineItemCategory(item) {
    ...
    return event === "labeled" || event === "unlabeled" ? "labels" : event === "renamed" ? "title" : event === "milestoned" || event === "demilestoned" ? "milestone" : event === "assigned" || event === "unassigned" ? "assignees" : event === "review_requested" || event === "review_request_removed" ? "reviewers" : event === "referenced" || event === "cross-referenced" || event === "connected" || event === "disconnected" ? "referenced" : null;
  }
  ...
  function shouldFetchSystemTimelineEvents(visibility) {
    ...
    return vis.labels !== !1 || vis.title !== !1 || vis.milestone !== !1 || vis.assignees !== !1 || vis.reviewers !== !1 || vis.referenced !== !1;
  }
  function needsLazyTimelineEventsFetch(prevVisibility, nextVisibility, timelineEvents) {
    ...
    if (shouldFetchSystemTimelineEvents(prevVisibility)) {
      ...
      if (!TIMELINE_CATEGORY_IDS.some(
        (id) => id !== "comments" && prev[id] === !1 && next[id] !== !1
      )) return !1;
    }
...
async function fetchPrTimelineEvents(owner, repo, number, fetchImpl, token = null, ctx = null) {
  ...
  try {
    ...
    if (lastPage == null) {
      ...
      for (; page <= 10 && parseLinkNextUrl(link); ) {
    ...
    const endPage = Math.max(1, Number(lastPage) || 1), startPage = Math.max(1, endPage - 10 + 1), cachedPage1 = startPage === 1 ? firstBatch : null;
...
async function fetchReactableReactors(nodeId, fetchImpl, token, ctx = null, opts = null) {
  ...
  return (await fetchReactableReactionGroups2(
    [id],
    fetchImpl,
    token,
    ctx,
...
async function fetchPullReviewThreadsBundle(owner, repo, pullNumber, fetchImpl, token, opts = {}) {
  ...
  const lastPageSize = Math.min(
  ...
  ), startPageSize = Math.min(
    20,
    Number(opts.startPageSize) || 20
  ), newest = await fetchReviewThreadsPage(
    owner,
    repo,
    pullNumber,
    {
    ...
    token,
    ctx
  ), totalCount = Number(newest.totalCount) || newest.threads.length;
  let oldest = null;
  ...
  const threads = [...newest.threads || []], comments = [...newest.comments || []], newestIds = newest.threads.map((t) => t.threadNodeId).filter(Boolean), oldestIds = [];
  if (oldest) {
    for (const t of oldest.threads || [])
      newestIds.includes(t.threadNodeId) || (threads.push(t), oldestIds.push(t.threadNodeId));
    for (const c of oldest.comments || [])
      comments.some((x) => String(x.id) === String(c.id)) || comments.push(c);
  }
  const loaded = threads.length, hiddenCount = Math.max(0, totalCount - loaded), meta = {
    totalCount,
    hiddenCount,
    ...
    hasOlder: !!newest.hasPreviousPage,
    ...
    hasNewerFromOldest: !!oldest?.hasNextPage,
...
function mergeReviewThreadsPageIntoDetail(detail, page, direction = "older") {
  ...
  let newestIds = new Set((prevMeta.newestThreadIds || []).map(String)), oldestIds = new Set((prevMeta.oldestThreadIds || []).map(String));
  const pageIds = (page?.threads || []).map((t) => t.threadNodeId).filter(Boolean).map(String);
  let newestStartCursor = prevMeta.newestStartCursor, newestEndCursor = prevMeta.newestEndCursor, hasOlder = prevMeta.hasOlder, oldestStartCursor = prevMeta.oldestStartCursor, oldestEndCursor = prevMeta.oldestEndCursor, hasNewerFromOldest = prevMeta.hasNewerFromOldest;
  if (!(dir === "refresh" || dir === "ids"))
    if (dir === "newest" || dir === "older") {
      for (const id of pageIds) newestIds.add(id);
      page?.startCursor && (newestStartCursor = page.startCursor), dir === "newest" && page?.endCursor && (newestEndCursor = page.endCursor), hasOlder = !!page?.hasPreviousPage;
    } else {
      for (const id of pageIds) oldestIds.add(id);
      page?.endCursor && (oldestEndCursor = page.endCursor), dir === "oldest" && page?.startCursor && (oldestStartCursor = page.startCursor), hasNewerFromOldest = !!page?.hasNextPage;
    }
  ...
  ) : reviewThreads.length : typeof page?.totalCount == "number" ? page.totalCount : Number(prevMeta.totalCount) || reviewThreads.length, loadedThreadCount = reviewThreads.length, hiddenCount = isRest ? page?.hasMore ? Math.max(1, totalCount - loadedThreadCount) : 0 : Math.max(0, totalCount - loadedThreadCount);
  for (const id of newestIds) oldestIds.delete(id);
  const restHasMore = isRest && !!page?.hasMore, meta = {
    ...prevMeta,
    totalCount,
    hiddenCount,
    loadedThreadCount,
...
async function fetchPrDetail(owner, repo, pullNumber, fetchImpl, token = null, opts = {}) {
  ...
  if (pr.node_id && token)
    try {
      const enriched = (await fetchReactableReactionGroups2(
        [pr.node_id],
        fetchImpl,
        token,
        ctx
