...
const RELOAD_REFRESH_MSG = "Extension was reloaded. Refresh this GitHub tab (\u2318R / Ctrl+R) to reconnect pr+.";
...
function cancelFetches(requestIds, opts = {}) {
  const ids = Array.isArray(requestIds) ? requestIds.map(String).filter(Boolean) : requestIds != null ? [String(requestIds)] : [];
...
async function send(message, { retries = 4, signal = null } = {}) {
  ...
  const requestId = message && message.requestId || (signal ? nextRequestId() : null);
  ...
  if (requestId && signal) {
    try {
      if (!signal.__prpRequestIds) signal.__prpRequestIds = /* @__PURE__ */ new Set();
  ...
  const logFetch = /FETCH|DETAIL|THREADS|COMMITS|CHECKS|DEVELOPMENT|COMMENTS|REVIEWS|FILES|COMPARE/i.test(
    msgType
  ) && !/PING|CANCEL/i.test(msgType);
  const tBridge0 = typeof performance !== "undefined" && performance.now ? performance.now() : Date.now();
  if (logFetch) {
    console.log(
      `[pr-plus][bridge] start ${msgType}` + (payload?.owner && payload?.repo ? ` ${payload.owner}/${payload.repo}` : "") + (payload?.number != null ? `#${payload.number}` : "") + (payload?.headSha ? ` sha=${String(payload.headSha).slice(0, 7)}` : "")
    );
  }
  let lastErr;
  try {
    for (let attempt = 0; attempt <= retries; attempt++) {
      ...
      try {
        ...
        const response = signal ? await Promise.race([sendP, whenAborted(signal)]) : await sendP;
        if (signal?.aborted) throw makeAbortError();
        if (response?.aborted) throw makeAbortError();
        if (logFetch) {
          const ms = Math.round(
            (typeof performance !== "undefined" && performance.now ? performance.now() : Date.now()) - tBridge0
          );
          const ok = response?.ok !== false;
          console.log(
            `[pr-plus][bridge] end ${msgType} ${ms}ms${ok ? " ok" : " fail"}` + (response?.error ? ` err=${String(response.error).slice(0, 80)}` : "")
          );
        }
        return response;
      } catch (e) {
...
function dropReviewThreadsFromDetailLocal(detail, threadNodeIds) {
  ...
  const prevDeleted = detail._deletedReviewCommentIds instanceof Set ? detail._deletedReviewCommentIds : Array.isArray(detail._deletedReviewCommentIds) ? detail._deletedReviewCommentIds : [];
...
function mergeReviewThreadsPageIntoDetailLocal(detail, page, direction = "older") {
  ...
  const missingIds = [.../* @__PURE__ */ new Set([...explicitMissing, ...derivedMissing])];
...
var PRTreeFetch = {
  ...
  async fetchPrDetail(owner, repo, number, opts = {}) {
    ...
    console.log(
      `[pr-plus] fetchPrDetail page round-trip ${owner}/${repo}#${number}: ${roundTrip}ms skipReviewThreads=${Boolean(opts.skipReviewThreads)} ` + (timings ? JSON.stringify(timings) : "(no per-request timings)")
    );
    return res.detail;
  },
  /**
  ...
  async fetchTagsForCommits(owner, repo, shas, opts = {}) {
    const want = new Set(
      (Array.isArray(shas) ? shas : []).map((s) => String(s || "").trim().toLowerCase()).filter(Boolean)
    );
    const res = await send(
      {
    ...
    if (res?.aborted) throw makeAbortError();
    if (res?.error && /unknown type:\s*PR_TREE_FETCH_TAGS_FOR_COMMITS/i.test(String(res.error))) {
      try {
        ...
        return (Array.isArray(all) ? all : []).filter(
          (t) => want.has(String(t?.sha || "").toLowerCase())
        );
      } catch {
      }
    }
  ...
  async fetchPrFiles(owner, repo, number, opts = {}) {
    ...
    return {
      ...
      gitattributesText: typeof res.gitattributesText === "string" ? res.gitattributesText : ""
    };
  },
  ...
  async setIssueSubscription(owner, repo, number, { subscribed = true, ignored = false, nodeId = null } = {}) {
    const res = await send({
...
try {
  ...
  if (root) {
    ...
    root.setAttribute(
      ...
      typeof PRTreeStorage?.getGithubTokenStatus === "function" || typeof PRTreeStorage?.getExtensionPrefs === "function" ? "1" : "0"
    );
  }
} catch {
}
